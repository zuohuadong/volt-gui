...
- 使用 `agmesh automation review-loop . --task <id> --domain delivery --panels contract,tests,runtime --max-rounds 3` 生成有界计划；coordination DB v2 项目写入 `review_loops` 表，legacy 项目才生成 `.agents/state/review-loops/<task>.json`。该命令不自动启动模型，panel 执行仍走 `agmesh subagent dispatch`
...
- `review-loop` 计划会写入 additive `adaptive_depth` 元数据，说明 min/max rounds、early-exit、deepen、escalate 和 contraction 判据；`review-loop-run --json` 在既有 `trace_eval` 字段外追加 `executed_rounds`、`early_exit_reason`、`contraction_delta` 和 `adaptive_depth`。这些字段只解释循环是否稳定、是否需要继续有界轮次或升级到强审查/人工判断，不覆盖 5 轮上限、release gate、approval、product-signal 或真实生产边界。
...
- `review-loop` / `evaluator-optimizer` 最多 6 个 panel、5 轮；agent score 只是代理指标，不是 CTR/CVR、付费、留存或生产真相。需求发现、营销获客和商业方向必须用 `agmesh automation product-signal` 记录真实世界数据或人工裁决；生产、secret、破坏性 git、迁移和发布必须走 `agmesh approval request|approve|reject`；同类失败出现 2-3 次时用 `agmesh automation skill-evolution --write` 生成 Matter 草案，不能自动改 skill/runbook
