...
```text
针对每个配置好的工作区执行 agent-team 自动化调度。先做轻量扫描，只读取判断队列是否有动作所需的最小信息：`tasks.md` active 表格、最近 progress 条目、`.mailbox/` frontmatter、`.agents/state/tasks.json` 和必要状态文件；只有命中可处理项后，才读取当前 task contract、命名 mailbox 证据、对应 workflow 和相关 skills。不要把完整 `progress.md`、完整 `.mailbox/` 历史或 archived task contracts 塞进提示词；需要历史证据时优先用 `agent-team memory recall "<query>" --token-budget <n>` 或点名读取 archive 文件。若 `agent-team automation doctor .` 报 coordination context warning，先运行建议的 `archive-ledger` / `archive-progress` / `prune-mailbox` 再做宽范围执行。只把项目级 Task Ledger 作为执行源。
