...
6. 通过 `agmesh automation sweep-record . --generated <task_id,...> --claim-after <iso> --force` 更新 sweep 状态：coordination DB v2 写入 `sweep_state`，legacy 项目写 `.agents/state/sweep.json`，使 `automation claim` 能拒绝同轮领取
...
```md
| ATC-0NN | local | <repo> | backlog-sweep | <一句话标题> | normal | low | ready | - | - | - | - | codex/atc-0NN-<slug> | - |
...
仍需人工纪律：sweep 生成候选时必须通过 `sweep-record` 同步更新 sweep state，否则 claim 侧只能执行 deny-list 兜底，无法识别同轮生成关系。每次发布前运行 `agmesh automation release-check` 验证 deploy、测试和状态模板完整性。
