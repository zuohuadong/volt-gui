...
- 压缩/召回方案变更进入默认路径前，先运行 `agmesh automation promotion-gate .`；`/token-report` 只提供用量趋势，不替代 memory benchmark 的命中率和 token 缩减门禁
- 排查单个 Codex 会话上下文膨胀时，使用 `agmesh automation inspect-session-context <session-id|session-file>`；该命令只输出热点摘要，不读取或打印原始 API 日志。
...
2. 运行：
   ```bash
   agmesh automation inspect-session-context <session-id-or-jsonl> --top 12
   ```
