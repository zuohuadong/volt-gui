# Contributing to VoltUI

Thank you for your interest in contributing to VoltUI! This guide covers
...
```bash
git clone https://github.com/zuohuadong/volt-gui.git
cd volt-gui
...
| `internal/memory` | VOLTUI.md / legacy REASONIX.md hierarchy + auto-memory |
...
with `VOLTUI_HOME` set. This gives each build its own self-contained directory
...
```bash
VOLTUI_HOME=/tmp/voltui-dev go run ./cmd/voltui
# or after building:
#   VOLTUI_HOME=/tmp/voltui-dev ./bin/voltui
```
...
VOLTUI_HOME=/tmp/voltui-dev-isolated build/bin/voltui-desktop
```

On Windows, use `$env:VOLTUI_HOME` in PowerShell or `set VOLTUI_HOME=` in
...
logs — stays under `VOLTUI_HOME`. Legacy migration, OS-home convention
directory scanning, and all other fallback paths are skipped so no production
data leaks in or out.

### Cache-first review gate

VoltUI treats high prompt-cache hit rate as product behavior. Changes that
