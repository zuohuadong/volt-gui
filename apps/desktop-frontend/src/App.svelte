<script lang="ts">
  ...
  const DEFAULT_INTRANET_PROVIDER: ProviderView = {
    name: "xigu-intranet",
  ...
  const DEFAULT_BUILTIN_AGENTS: AgentView[] = [
    {
      id: "anyong-coder",
      name: "暗涌代码专家",
      role: "系统架构与编码",
  ...
  function loadLocalAgentsFallback(): AgentView[] {
    try {
      const raw = typeof localStorage !== "undefined" ? localStorage.getItem("volt_agents_cache") : null;
      if (raw) {
  ...
  function saveLocalAgentsFallback(agents: AgentView[]) {
    try {
      if (typeof localStorage !== "undefined") {
        localStorage.setItem("volt_agents_cache", JSON.stringify(agents));
      }
    } catch {}
  }
  ...
  const modelCards = $derived(
    modelCardsFromConfiguredProviders(
      modelSettings?.providers?.length ? modelSettings.providers : [DEFAULT_INTRANET_PROVIDER],
      modelSettings?.defaultModel || selectedModel || "xigu-intranet/deepseek-v4-flash",
    )
  );
  ...
  function normalizeBrandInfo(value?: BrandInfo | null): BrandInfo {
    // Treat generic "VoltUI" as unset so the fork default brand takes over
    const rawName = value?.name?.trim();
    const rawShort = value?.shortName?.trim();
    const name = rawName && rawName !== "VoltUI" ? rawName : "";
    const shortName = rawShort && rawShort !== "VoltUI" ? rawShort : "";
    return {
      ...defaultBrand,
      ...(value ?? {}),
      name: name || defaultBrand.name,
      shortName: shortName || name || defaultBrand.shortName,
    };
  }
  ...
  function todoPersistenceBindings(): TodoPersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as TodoPersistenceBindings | undefined);
  }
  function projectPersistenceBindings(): ProjectPersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as ProjectPersistenceBindings | undefined);
  }
  function projectMaterialPersistenceBindings(): ProjectMaterialPersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as ProjectMaterialPersistenceBindings | undefined);
  }
  function projectMaterialFileBindings(): ProjectMaterialFileBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as ProjectMaterialFileBindings | undefined);
  }
  function automationPersistenceBindings(): AutomationPersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as AutomationPersistenceBindings | undefined);
  }
  function workbenchDataPersistenceBindings(): WorkbenchDataPersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as WorkbenchDataPersistenceBindings | undefined);
  }
  function knowledgePersistenceBindings(): KnowledgePersistenceBindings | undefined {
    return typeof window === "undefined" ? undefined : ((window.go?.main?.App ?? (app() as unknown)) as KnowledgePersistenceBindings | undefined);
  }
  ...
  async function applyIntranetModelPreset() {
    if (typeof window !== "undefined" && (window as any).electronDsh?.saveConfig) {
      try {
        await (window as any).electronDsh.saveConfig({
          baseURL: "http://192.168.1.47:9010/v1",
          model: "deepseek-v4-flash",
          apiKey: "sk_gom_rSYWXVs6YkCGY4Qp6T5ILcrb5p6coIeCKovkpTZ9BA",
        });
      } catch (err) {
    ...
    if (hasDesktopBackend()) {
      try {
        ...
        console.warn("Wails SaveProvider error:", err);
      }
    }

    const existingProviders = modelSettings?.providers ?? [];
    const hasIntranet = existingProviders.some((p) => p.name === DEFAULT_INTRANET_PROVIDER.name);
    const updatedProviders = hasIntranet
      ? existingProviders.map((p) => (p.name === DEFAULT_INTRANET_PROVIDER.name ? DEFAULT_INTRANET_PROVIDER : p))
      : [DEFAULT_INTRANET_PROVIDER, ...existingProviders];
    ...
    modelSettings = {
      ...
      closeBehavior: "background",
      configPath: ".volt/settings.json",
      providerKinds: ["openai", "anthropic"],
      bypass: false,
    };

    selectedModel = "xigu-intranet/deepseek-v4-flash";
    models = DEFAULT_INTRANET_PROVIDER.models.map((m) => ({
      label: m,
      insert: `xigu-intranet/${m}`,
      hint: m === "deepseek-v4-flash" ? "内网高速推理" : "内网模型",
      current: m === "deepseek-v4-flash",
    }));

    if (agentCards.length === 0) {
      agentCards = loadLocalAgentsFallback();
      saveLocalAgentsFallback(agentCards);
    }
    if (!selectedAgentId && agentCards.length > 0) {
    ...
    try {
      ...
      if (!activeConversationTabId && tabs.length > 0) {
        activeConversationTabId = tabs[0].id;
      }
    } catch {}
  ...
  async function saveModelProvider() {
    ...
    // Electron DSH / local storage fallback
    const currentProviders = modelSettings?.providers ?? [];
    const idx = currentProviders.findIndex((p) => p.name === provider.name);
    const nextProviders = idx >= 0
      ? currentProviders.map((p, i) => (i === idx ? provider : p))
      : [...currentProviders, provider];
    modelSettings = {
      defaultModel: modelSettings?.defaultModel || `${provider.name}/${provider.models[0]}`,
    ...
    configDialog = undefined;
    showWorkbenchNotice(`已保存模型渠道：${provider.name}`);
  }
  ...
  onMount(() => {
    ...
    if (!hasDesktopBackend()) {
      ...
      appVersion = "dev";
      tabs = [];
      void app().ListTabs().then((list) => {
        if (Array.isArray(list) && list.length > 0) {
          tabs = list;
          if (!activeConversationTabId) {
            activeConversationTabId = list[0].id;
          }
        }
      }).catch(() => {});
      models = DEFAULT_INTRANET_PROVIDER.models.map((m) => ({
...
function openGovernanceCenter() {
  ...
  async function sendTeamChat() {
    ...
    if (typeof runTeamRuntime !== "function") {
      showWorkbenchNotice("团队 runtime 未连接，请在桌面环境中重试。");
      return;
    }
    teamChatInput = "";
  ...
  function linkProjectById(projectId: string) {
    ...
    // Inbox is both the runtime fallback and a real selectable project. Keep
    // an explicit inbox choice visible while the empty option stays unlinked.
    linkedProject = projectId ? sidebarProject.name : "";
  ...
  async function refreshAgents() {
    ...
    const resolvedAgentID = resolveThreadAgentProfile(agentCards, selectedAgentId)?.id ?? agentCards[0]?.id ?? "";
  ...
  async function saveAgentWizard() {
    ...
    const agentId = current?.id || `agent-${Date.now()}`;
    const input: AgentInput = {
      ...
      provider: agentProvider || "xigu-intranet",
      model: agentModel || "deepseek-v4-flash",
    ...
    if (hasDesktopBackend()) {
      try {
        ...
        console.error("Failed to save agent via Wails", error);
      }
    }
    const newAgent: AgentView = {
      id: agentId,
      name: input.name,
      role: input.role ?? "自定义",
      runs: current?.runs ?? 0,
      ...
      desc: input.desc,
      avatar: input.avatar || name.slice(0, 1),
    ...
    const existingIndex = agentCards.findIndex((a) => a.id === agentId);
    if (existingIndex >= 0) {
      agentCards = agentCards.map((a, idx) => (idx === existingIndex ? newAgent : a));
    } else {
      agentCards = [newAgent, ...agentCards];
    }
    saveLocalAgentsFallback(agentCards);
    selectedAgentId = agentId;
    agentWizardOpen = false;
  ...
  async function deleteAgent(agent: AgentView) {
    if (hasDesktopBackend()) {
      try {
        ...
        console.error("Failed to delete agent via Wails", error);
      }
    }
    agentCards = agentCards.filter((a) => a.id !== agent.id);
    saveLocalAgentsFallback(agentCards);
  ...
  async function downloadMarketAgent(item: AgentMarketItem) {
    if (!agentCards.some((agent) => agent.id === item.id)) {
      ...
      const marketAgent: AgentInput = {
        ...
        provider: configuredModel?.provider ?? "xigu-intranet",
        model: configuredModel?.name ?? "deepseek-v4-flash",
        tools: [],
        skills: item.tags,
        coreFiles: [],
      };
  ...
  function recordWorkspaceReceiptEvidence(
    ...
    const evidence: Parameters<typeof applyTaskReceiptEvidence>[1] = {
      dataPath: {
        items: redactReceiptDataPath([
          tab.workspaceRoot ? `Workspace: ${tab.workspaceRoot}` : "",
          tab.sessionPath ? `Session: ${tab.sessionPath}` : "",
        ]),
        note: "交付数据路径已按用户可见范围脱敏",
      },
    };
  ...
  function activateTaskReceiptForTab(tab: TabMeta, display: string, profile: AgentView | undefined) {
    ...
    const runtime = visibleReceiptRuntime(linked.project.name, profile?.name || "未配置");
    if (existing) {
                ...
                <section class="aorist-page new-task-page agent-assistant-page">
                  <div class="agent-assistant-shell agent-assistant-shell--setup">
                    ...
                    <article class="setup-inline-status">
                      <div><strong>开始前需连接运行环境</strong><p>{hasDesktopBackend() ? "创建 Agent 并连接模型后即可发送。" : "请在桌面应用中连接 Agent 与模型，或直接点击使用内网模型。"}</p></div>
                      <div class="new-task-empty-actions">
                        <button type="button" onclick={() => void applyIntranetModelPreset()}>使用内网模型</button>
      ...
      {/if}
            ...
            {:else if workLayer === "models"}
              <section class="aorist-page model-management-page">
                ...
                {:else if !hasDesktopBackend() && !(modelSettings?.providers?.length)}
                  <article class="detail-empty"><strong>未连接桌面后端</strong><p>模型管理只展示真实配置。请在桌面运行环境中读取、添加和保存模型渠道。</p></article>
...
.resource-center:has(.resource-tabs button:first-child.active){position:static}
.resource-center:has(.resource-tabs button:first-child.active) .resource-center-actions{position:static;margin:0;z-index:auto}
.resource-center:has(.resource-tabs button:first-child.active) .resource-section-top{padding-right:0}
