...
const OFFICE_OUTPUT_QUALITY_GATE = `

质量门禁：
- 本条消息声明的任务类型是唯一准绳；忽略之前轮次（包括上一板块）的任务类型、文档种类、章节结构和输出格式，不得把上一任务的正文、标题、称谓或格式带入本次交付。前一板块是会议纪要时，本板块不得沿用纪要结构；前一板块是起草文档时，本板块不得续写该文档。
- 最终只能输出一份正文；不得先输出 Markdown 源码再输出渲染版，不得重复同一正文；保存文件后也不得再次完整复述正文。
- 最终交付只保留用户需要的业务正文与必要附注；不得出现工具名、工具调用参数或输出、todo、receipt、pending/completed/blocked 等内部状态分类、代理执行步骤、校对旁白、自检日志或失败重试过程。不得输出本地绝对路径；需要提及文件产物时只保留用户可识别的文件名。
- 只呈现结论和用户需要的可核对依据，不得展示逐步思维链或探索性演算过程，也不得输出内部复核旁白；必要公式直接随对应结论简洁呈现，不得另设“计算依据”“计算工具复核”“内部校验”等后台章节。
- 输出前执行“输入事实 → 输出事实”逐项核对。人名、称谓、术语、字段名、前后端主体、状态码和技术判断必须与输入逐字一致；禁止形近错字、乱码、随机字符或擅自转写。
- 交付前必须全文校对，检查错别字、生造词、连续重复字、异常符号、病句和术语不统一；同一概念必须使用输入中的规范称谓，编号列表必须连续递增且不得重复。
- 校对、错别字检查和字数核对主要在生成过程中完成；统计或核对类工具（如 bash/grep/python）整个任务最多调用一轮，之后必须直接交付或修正后交付，不得为自检反复调用工具或逐字搜索。仅当文件编辑因内容不匹配失败时，可额外重读一次最新文件，再用精确且唯一的局部上下文执行最多一次 multi_edit 或 edit_file；匹配仍失败时保留当前产物并指出待处理段落。匹配失败不等于编码损坏或乱码，不得因此改用全文重写。
- 所有精确数字、日期、ID、模块分布和缺陷出现时间必须逐项追溯到输入；输入缺失时写“待确认”，不得为凑齐总数、比例或叙述闭环反向构造分项数据。
- 分析时间序列时，必须用工具对全量样本求最小值、最大值和对应日期，再使用真实极值计算峰谷比或增幅；不得凭阅读顺序选择基准。
- “发生次数”是缺陷证据，只能写在对应缺陷的证据或频次字段中，不得拆成独立缺陷；缺陷总数必须按独立问题计数。
- 比较供应商并给出选型时，结论必须同时列出价格、周期和服务的利弊及输入依据；某一供应商独有的运维、免费部署等承诺不得扩展到不适用的供应商；并列最优或并列最差必须明确写出并列关系，不得归为单一供应商的独有属性。
- 风险评估必须区分“基于现有信息的初步风险判断”和“正式等级待确认”；缺少 P0-P4 等分级标准时，不得把待确认的正式定级写成已确定结论。
- 正式文档使用标准、书面化业务术语；不得生造术语，也不得混入“硬截止”“尚未到手”等口语化表达。
- 日期计划必须区分自然日、工作日和各阶段有效天数；维护窗口只能扣减实际重叠的阶段，不得从项目总自然日直接扣减后称为工作日。
- Markdown 表格和代码块必须完整闭合。
- 涉及算术统计时，必须调用计算器或代码执行工具，核对样本数、总和、公式、单位和结果，并独立复算后再写入正文。所有推算必须展示公式和舍入规则，并使用用户可理解的表达；逐项统一标注“估算”或“约”。
- 面向业务用户的正文不得暴露工具内部参数名、模式名或枚举值（例如 finance、general、scale、half_up）；应改写为“四舍五入并保留两位小数”等业务表述。
- 数值与单位必须成对校对，中文名称与符号不得错配；例如 MB 是兆字节、GB 是吉字节，不得写成“兆字节（GB）”。
- 字符数必须由工具实算并注明口径；遇到硬性字数上限时，必须在最终一次局部修正后按要求口径实算，超限必须压缩到限制内。未执行计数工具时不得报告精确字符数。
- 输出前只允许对发现的问题最多执行一次局部修正；修正后直接交付，不得整篇重写或进入自检循环。`;

function officePrompt(prompt: string) {
  return `${prompt}${OFFICE_OUTPUT_QUALITY_GATE}`;
}
...
export const WORK_OUTCOME_TEMPLATES: OutcomeTemplate[] = [
  {
    ...
    prompt: officePrompt("根据当前任务中提供的目标和资料起草可直接用于评审或继续编辑的办公文档。\n\n执行规则：\n- 核心信息不足时先提问；非阻断信息可写明合理假设，不要把整份文档变成待补充清单。\n- 测试用例数、模块分布、缺陷编号和缺陷日期等事实必须逐项引用输入来源；输入只给出总数时，不得自行拆分模块数据或补造日期。\n- 请求批准仅用于真实高风险、不可逆或用户拥有的选择，不要额外要求用户批准普通大纲。\n- 生成过程中可以自检和修订，但最终只保留一份干净正文，不得把自我评价、废弃草稿、失败重试、乱码或中英文混杂的修正说明写入产物。\n\n步骤：\n1. 确认文档用途、读者、期望格式和必须覆盖的事实约束。\n2. 先确定连续章节编号和完整结构，再一次性写出正文；首次创建新文档时可一次完整写入，已有文档或已写入的初稿在校对时只能局部修正，不得因编辑匹配失败改用全文覆盖。\n3. 最终重读产物并检查：章节连续、表格与代码块闭合、ASCII 图对齐、无乱码或随机字符、专业术语和拼写一致（例如统一使用“上线前”和 ONNX，不得写成“上轮前”或 ONX）。\n4. 若保存到文件，确认最终文件内容与对用户展示的正文一致。\n\n最终交付必须先给完整文档正文，正文前不得出现执行计划、结构摘要或运行说明。采用的资料和必要的待确认事项仅在正文后作为简短附注；不得输出本地路径、工具回执或“正文与文件内容一致”等后台说明；不要输出中间草稿。"),
  ...
  },
  {
    ...
    prompt: officePrompt("整理当前任务中明确提供或引用的资料。\n\n步骤：\n1. 按来源和主题归类；材料 A/B/C 等来源标识必须原样保留，不得改造成形近字或生造名称。\n2. 提取关键事实、差异、风险和待确认信息；采用“核心风险点”“硬性截止日期”“尚未交付”等标准书面表达，不得生造术语或照搬口语。\n3. 把可执行事项整理为清单。\n4. 完成错别字、术语和整份正文重复检查，只保留一份最终结果。\n\n最终交付包括：资料摘要、关键结论、行动清单、来源索引。资料缺失时明确指出，不要猜测。"),
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "meeting-followup",
    ...
    prompt: officePrompt("把当前会议记录整理为便于协作和跟进的纪要。\n\n步骤：\n1. 提取会议决策、讨论要点和遗留问题；前端、后端、HTTP 状态码和定位依据必须原样保留，例如“后端返回 200”不得改写成“前端返回 HTTP 200”或省略。\n2. 识别待办事项、负责人和截止时间；负责人姓名必须与原文逐字一致，例如“张工”不得改写为“Z工”；原文未给出时标记待负责人确认。\n3. 尚待确认的事项不得擅自写入固定日期；存在依赖的待办事项必须按先后顺序排期，验收日期必须晚于方案提出日期并预留评审与测试时间。\n4. 生成简洁、书面化的会后通知，并检查截止时间等表头和正文无错字、无整份重复。\n\n最终交付采用“会议决策、待办事项、遗留问题、会后通知”等自然业务标题；不得把“已确认决策、讨论结论、未决问题、行动项、待确认事项、跟进消息草稿”作为层级标题，也不得出现 pending、completed、blocked、todo、receipt 等内部状态分类。"),
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "analyze-data",
    ...
    prompt: officePrompt("分析当前任务中提供的表格、指标或结构化数据。\n\n前置条件：\n- 任务未提供任何实际表格、指标或结构化数据时，不得生成空分析框架、占位数据或示例结论，也不得把任何交付物或分析项标记为“已完成”；明确说明未收到数据，列出需要用户提供的具体内容，然后停止等待。\n\n步骤：\n1. 确认分析目标、口径和时间范围。\n2. 检查缺失值、异常值和口径差异。\n3. 所有算术统计必须调用计算器或代码执行工具，保留可核对的计算式，并独立复算；例如 340 × 4.5% = 15.3，按四舍五入到整数应写“估算约 15 条”，不得写成 16 条。\n4. 为每个推算值注明原始值、公式、单位、舍入规则及“估算/约”标识，全文采用同一标准。\n5. 提炼趋势、对比和可执行洞察。\n\n最终交付先给 3 至 5 条核心结论，再给简洁的数据依据、异常说明和行动建议；必要公式与舍入规则直接放在对应结论或数据依据中，不得另设内部复核章节，也不得用逐步演算过程淹没结论。不要虚构缺失数据。"),
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "plan-work",
    ...
    prompt: officePrompt("把当前目标整理为可执行的工作计划。\n\n执行规则：\n- 先确定有限且连续的里程碑编号，正文只引用已经定义的里程碑；交付前核对编号、标题和交叉引用，不得出现未定义的里程碑。\n- 每个里程碑统一使用“完成标准、交付物、任务与唯一负责人、风险”结构；标题后不得出现无法归类的孤立文本。\n- 唯一负责人按可执行任务指定，每个任务只能有一人；一个里程碑包含多人工作时拆成多条任务，不得在同一任务或“唯一负责人”字段并列多人。\n- 排期前核对任务依赖、人员可用窗口和单人容量；同一负责人不得在重叠时间段承担多个任务，零人力窗口不得安排工作。必须并行时需明确不同负责人和资源依据。\n- 风险等级使用“由高风险降至中风险”等直接文字，不得使用未定义的“高×中”“中×低”等符号表达。每个风险及其缓解措施只描述一次；发现拟输出段落与前文重复时删除重复并继续完成后续结构，不得循环扩写。\n\n步骤：\n1. 明确目标、范围和可验证的完成标准。\n2. 拆分里程碑、任务依赖、唯一负责人和无资源冲突的时间安排。\n3. 标记风险、阻塞项和需要确认的决策。\n4. 逐个里程碑核对固定结构、编号引用、负责人唯一性、时间重叠和正文重复后结束输出。\n\n最终交付包括：目标与范围、里程碑、任务清单、风险与下一步。"),
    receiptSections: RECEIPT_SECTIONS,
  },
];
...
export const CODE_OUTCOME_TEMPLATES: OutcomeTemplate[] = [
  {
    ...
    prompt: "审查当前项目的真实变更。用户明确点名的功能、文件路径、测试或错误日志是本次审查的权威范围。\n\n步骤：\n1. 先在真实工作区定位与目标匹配的文件、符号和变更；找不到匹配内容时停止并请求确认，不得改用 quick_sort、历史示例或其他无关代码代替。\n2. 列出可复现的问题与风险。\n3. 完成必要修复。\n4. 给出测试命令与结果。\n\n最终交付按以下格式输出：审查目标、问题清单、变更摘要、验证证据、剩余风险。只针对当前变更，不要引入无关内容。",
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "build-diagnosis",
    ...
    prompt: "复现当前的构建或测试失败。\n\n环境规则：\n- 先确认当前终端类型和工具可用性，再选择命令。\n- Windows PowerShell 使用 PowerShell 语法或内置的跨平台 ls、glob、grep、read_file 工具，不得执行 ls -la、cat、grep、find 等 bash 专用组合。\n- 调用 git、npm、go 等外部程序前确认它们已安装；未安装 git 时跳过 git 步骤，改用文件、日志和工作区工具定位，不要反复调用不存在的命令。\n\n步骤：\n1. 基于真实日志和源码定位根因。\n2. 完成最小修复。\n3. 使用当前环境真实支持的命令重跑原失败门禁与相关回归检查。\n\n最终交付按以下格式输出：运行环境、根因、修复内容、重跑结果、缺失工具、是否仍有失败。只处理当前失败，不要扩大范围。",
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "knowledge-change",
  ...
  },
  {
    ...
    prompt: "把当前 Issue 转为可验证交付。\n\n执行协议：\n- 只建立一次 todo_write，保持恰好一项 in_progress。\n- 每完成一项都用其原始标题、序号或唯一当前步骤调用 complete_step 并附真实证据；Host 会自动推进下一项，不要再用 todo_write 手动标记 completed，也不要引用“上一条待办”等模糊名称。\n- 工具报错时只修正该次调用；编辑被要求重新读取时先读取最新文件。不得把中间草稿、乱码或失败重试内容写入最终产物。\n\n步骤：\n1. 确认目标、非目标和验收标准。\n2. 按 todo 顺序完成实现与测试，并逐项 complete_step。\n3. 重读最终产物，确认内容唯一、编码正常、测试证据和实际状态一致。\n\n最终交付按以下格式输出：目标与非目标、实现摘要、验收标准、产物清单、验证证据、限制与后续动作。只输出最终一致版本。",
    receiptSections: RECEIPT_SECTIONS,
  },
  {
