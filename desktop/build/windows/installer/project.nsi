...
reasonix_webview_interactive:
    MessageBox MB_OK|MB_ICONSTOP "Microsoft Edge WebView2 Runtime could not be installed online (exit code $1). On an offline or restricted network, download the separately versioned Windows prerequisites ZIP from https://cnb.cool/aizhuliren/volt/anyong-agent/-/releases, extract it, run install-prerequisites.cmd, then run this installer again."
    Abort "WebView2 Runtime installation failed with exit code $1."

reasonix_webview_silent:
    SetErrorLevel $1
    Abort "WebView2 Runtime installation failed. Install the separately versioned Windows prerequisites package first: https://cnb.cool/aizhuliren/volt/anyong-agent/-/releases"
