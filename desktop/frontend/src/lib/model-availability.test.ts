...
describe("model availability", () => {
  it("marks connection failures and clears the marker after recovery", () => {
    const empty = {};
  ...
  it("rejects a pending new-task model only when the live catalog removed it", () => {
    const models = [
      ...
      { ref: "gateway/removed", availability: "unavailable" as const, unavailableReason: "retired" },
    ];
    ...
    expect(pendingModelSelectionUnavailableReason(models, "gateway/removed")).toBe("retired");
    expect(pendingModelSelectionUnavailableReason(models, "gateway/missing")).toContain("no longer published");
  });
});
