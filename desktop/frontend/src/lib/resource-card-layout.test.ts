...
describe("resource card layout", () => {
  ...
  test("keeps knowledge status badges inside narrow cards", () => {
    expect(appComponent).toContain(".knowledge-template-card .knowledge-card-status{flex:0 0 auto;white-space:nowrap}");
    expect(appComponent).toContain(".knowledge-template-card .knowledge-card-type{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}");
  });
});
