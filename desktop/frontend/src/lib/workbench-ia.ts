...
export const WORK_OUTCOME_TEMPLATES: OutcomeTemplate[] = [
  ...
  },
  {
    ...
    prompt: officePrompt("把当前会议记录整理为便于协作和跟进的纪要。\n\n步骤：\n1. 提取已确认决策、讨论结论和未决问题；前端、后端、HTTP 状态码和定位依据必须原样保留，例如“后端返回 200”不得改写成“前端返回 HTTP 200”或省略。\n2. 识别行动项、负责人和截止时间；负责人姓名必须与原文逐字一致，例如“张工”不得改写为“Z工”；原文未给出时标记待确认。\n3. 待确认事项不得擅自写入固定日期；存在依赖的行动项必须按先后顺序排期，验收日期必须晚于方案提出日期并预留评审与测试时间。\n4. 生成简洁、书面化的会后跟进内容，并检查截止时间等表头和正文无错字、无整份重复。\n\n最终交付包括：会议结论、行动项、待确认事项、跟进消息草稿。"),
    receiptSections: RECEIPT_SECTIONS,
  },
  {
    id: "analyze-data",
