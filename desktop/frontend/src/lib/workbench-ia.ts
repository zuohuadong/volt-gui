...
- 校对、错别字检查和字数核对主要在生成过程中完成；统计或核对类工具（如 bash/grep/python）整个任务最多调用一轮，之后必须直接交付或修正后交付，不得为自检反复调用工具、逐字搜索或重读文件。文件编辑失败时改用整份重写一次完成，不得连续多次局部编辑。
