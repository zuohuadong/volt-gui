...
const OFFICE_OUTPUT_QUALITY_GATE = "\n\n质量门禁：\n- 最终只能输出一份正文；不得先输出 Markdown 源码再输出渲染版，也不得重复同一正文。\n- 人名、称谓、术语、字段名和数字必须与输入逐字一致；禁止形近错字、乱码、随机字符或擅自转写。\n- Markdown 表格和代码块必须完整闭合。\n- 涉及算术统计时，必须调用计算器或代码执行工具，核对样本数、总和、公式、单位和结果，并独立复算后再写入正文。";
