...
func (a *App) RefreshModelsForTab(tabID string) []ModelInfo {
	...
	if err != nil {
		return markModelCatalogUnknown(configured, "Model catalog is temporarily unavailable; showing static configuration.")
	}
...
func reconcileConfiguredModel(model ModelInfo, outcome modelCatalogProbeOutcome, probed bool) (ModelInfo, bool) {
	if !config.IsLikelyChatModel(model.Model) {
		return unavailableCurrentCatalogModel(model, "The current model does not support chat conversations.")
	}
	if !probed || outcome.err != nil {
		model.Availability = "unknown"
		model.UnavailableReason = "Model catalog refresh failed; showing static configuration."
		return model, true
	}
	...
	return unavailableCurrentCatalogModel(model, "The model gateway no longer publishes this model.")
}
