...
func TestRefreshModelsForTabUsesLiveProviderCatalog(t *testing.T) {
	...
	if len(models) != 1 || models[0].Ref != "local/chat-model" || models[0].Availability != "available" {
		t.Fatalf("RefreshModelsForTab() = %+v, want only the live chat model", models)
	}
}
...
func TestRefreshModelsForTabKeepsRemovedCurrentModelUnavailable(t *testing.T) {
	...
	writeModelCatalogTestConfig(t, server.URL+"/v1")
	models := modelCatalogTestApp("local/retired-model").RefreshModelsForTab("tab")
	if len(models) != 2 {
		t.Fatalf("RefreshModelsForTab() = %+v, want live chat model and removed current model", models)
	}
	for _, model := range models {
		if model.Current && (model.Ref != "local/retired-model" || model.Availability != "unavailable") {
			t.Fatalf("removed current model = %+v, want unavailable retired model", model)
		}
	}
}
...
func TestRefreshModelsForTabKeepsPublishedNonChatCurrentModelUnavailable(t *testing.T) {
	...
	for _, model := range models {
		if model.Current && (model.Ref != "local/image-gpu5" || model.Availability != "unavailable") {
...
func TestRefreshModelsForTabRetainsStaticModelsWhenProbeFails(t *testing.T) {
	...
	models := modelCatalogTestApp("local/chat-model").RefreshModelsForTab("tab")
	if len(models) != 2 {
		t.Fatalf("RefreshModelsForTab() = %+v, want all static chat models", models)
	}
	for _, model := range models {
		if model.Availability != "unknown" {
			t.Fatalf("static model = %+v, want unknown availability", model)
		}
	}
}
...
func writeModelCatalogTestConfig(t *testing.T, baseURL string) {
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Models: []string{"chat-model", "retired-model", "image-gpu5"}, Default: "chat-model", APIKeyEnv: "LOCAL_API_KEY",
	}}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)
	}
