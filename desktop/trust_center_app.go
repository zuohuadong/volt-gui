...
func trustFileEgressFlows(cfg *config.Config, activeModel string, im TrustEnterpriseIMView) []TrustFlow {
	...
	return []TrustFlow{
		...
		{ID: "egress:browser-upload", Category: "file-egress", Name: "浏览器文件上传", Status: TrustStatusDisabled, Direction: "本机文件 -> 浏览器页面", Classification: "unknown", DataCategories: []string{}, Detail: "当前 browser_control 动作未提供文件选择/上传能力。"},
		...
		{ID: "egress:mcp-tools", Category: "file-egress", Name: "MCP/工具联网", Status: remoteMCPStatus, Direction: "工具参数/本地文件引用 -> 工具目标", Classification: "unknown", DataCategories: []string{"工具明确接收的文本、路径或文件内容"}, Detail: "取决于具体 MCP/工具语义与调用参数；不伪造已发送事件。"},
	}
}
