...
func TestSettingsListsAndRemovesPermanentTrustedIntranetSite(t *testing.T) {
	...
	if _, err := cfg.AddTrustedIntranetSite("lims.volt.org", "192.168.1.14", 443); err != nil {
		t.Fatal(err)
	}
	...
	if site.Host != "lims.volt.org" || len(site.CIDRs) != 1 || site.CIDRs[0] != "192.168.1.14/32" || len(site.Ports) != 1 || site.Ports[0] != 443 {
