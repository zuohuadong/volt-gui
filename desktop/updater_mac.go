...
const macBundleID = "com.voltui.desktop"
...
func applyMac(zipPath, targetVersion string) error {
	...
	staging, err := os.MkdirTemp("", "voltui-mac-update-*")
	if err != nil {
		return err
	}
	handedOff := false
	...
	backupApp := currentApp + ".voltui-update-backup"
	...
	script := filepath.Join(staging, "install-voltui-update.sh")
...
# keep the old bundle registered and refuse to launch the replacement.
attempt=0
...
func findMacApp(root string) (string, error) {
	direct := filepath.Join(root, "VoltUI.app")
