...
QQ Bot 使用官方 QQ Bot 平台 API。它支持交互卡片按钮来完成审批
和问答，与飞书和 Lark 类似。当卡片按钮过期或平台提示操作失败时，
...
| QQ | 手动配置（App ID + App Secret） | 交互卡片按钮，也可用命令 | 交互卡片按钮，也可用命令 | QQ 群聊、个人会话和官方 QQ Bot 平台 |

飞书和 Lark 的卡片按钮会在后台转换为命令，例如 `/approve <id>`、
`/deny <id>` 或 `/answer <id> <选项>`。QQ 的卡片按钮也是如此。
...
- QQ 的 App ID 和 App Secret 保存在 Reasonix 的用户配置文件中。
