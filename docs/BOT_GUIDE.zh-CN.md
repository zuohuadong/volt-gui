...
访问控制仍然是必需项。你可以在 `[bot.allowlist]` 下为对应平台配置用户 ID，
有意设置 `allow_all = true`，或启用 `[bot.pairing]` 让未知私聊用户先收到
一次性配对码。配对码需要在本机执行 `reasonix bot pairing approve <code>`
后才会放行；群聊不会因为私聊配对自动开放，群 ID 仍是额外收窄条件，不能
替代用户白名单。

如果配置了 `qq_admins`、`feishu_admins`、`weixin_admins` 或对应
`*_approvers`，`/yolo`、`/mode` 等运行模式命令只允许 admin 使用，
`/approve` 和 `/deny` 只允许 approver 或 admin 使用。没有配置角色列表时，
为兼容旧配置，已允许的用户保持原有命令能力。远端用户进入的是同一个
Reasonix controller、权限策略、工具审批模式和沙盒边界，和本地桌面端或
CLI 回合一致。
