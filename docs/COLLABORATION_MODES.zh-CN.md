...
Reasonix 桌面端输入框左下角的“协作方式”菜单包含三类常用工作模式：
...
省 token 模式用于降低每轮请求中固定携带的上下文和工具 schema 开销。开启后，Reasonix 初始不会一次性带上完整技能索引、MCP、CodeGraph/LSP、部分内置工具等能力；当任务确实需要这些能力时，再按需启用。
...
- **担心 Reasonix 改错**：开启计划模式，必要时同时开启省 token。
