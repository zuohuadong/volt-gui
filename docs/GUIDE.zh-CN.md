...
# plan_mode_allow_host_automation = true        # 设为 false 后，计划模式阻断浏览器/桌面自动化
...
`[agent].plan_mode_allow_host_automation` 默认为 `true`，方便交互式计划阶段继续使用
`browser_control`、`desktop_keyboard`、`desktop_mouse`、`desktop_screenshot` 等第一方浏览器/桌面辅助能力。
如果在隐私敏感或共享机器上希望计划模式更严格，可设为 `false`。

`[agent].plan_mode_allowed_tools` 用于把 VoltUI 无法自动分类的自定义/外部工具声明为额外只读工具。
...
当计划阶段需要运行 VoltUI 尚不能自动分类、但你确认只读的 shell 查询命令时，使用
...
VoltUI 也可以提示你确认是否信任它为只读；选择持久信任会写入同一个
...
`VOLTUI_MEMORY_COMPILER_LLM_CLASSIFICATION=true` 会为 Memory v5 启用可选的 LLM 任务/聊天分类器。
默认关闭，此时 VoltUI 使用本地 heuristic classifier，不会产生额外 provider 调用。开启后，分类缓存未命中时，
VoltUI 可能先通过已配置 provider 发送一个很小的分类请求，再决定用户输入是任务还是普通对话；这会增加少量延迟、
...
| 终端原生选择 | 复制 transcript 文本 | VoltUI 默认不启用鼠标报告，因此终端自己的选择/复制仍可使用。 |
...
交互式 `voltui` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/memory-v5`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
`/memory` 会同时列出记忆文档（`VOLTUI.md` / 旧兼容 `REASONIX.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
Memory v5 在 CLI/TUI、`voltui serve` 和桌面端默认开启，因为这些入口共用同一套本地
controller。它会把本地、按项目隔离的执行轨迹和编译器状态写在 VoltUI home 下，并且只有
...
`voltui config memory-v5 off|observe|compact|on|status`。桌面端还可以在设置 → 通用 → Memory v5 中控制。
...
CLI/TUI 和 `voltui serve` 使用同一个 user/global 配置。项目内的 `voltui.toml` 不能覆盖
这个 user/global 设置。CLI 命令会更新底层配置；高级用户也可以手动编辑 VoltUI home 下的
用户配置：
...
`voltui run --metrics <path>` 时，JSON 还会输出内容无关的 `memory_compiler_*` 汇总字段，
...
完成。动态运行态只写进 `.voltui/autoresearch/...`，不写入 `VOLTUI.md`、旧兼容 `REASONIX.md`、`AGENTS.md`、
...
Planner 会看到已加载的 `VOLTUI.md` / 旧兼容 `REASONIX.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
`voltui config reasoning-language auto|zh|en`。Memory v5 使用 `/memory-v5 off|observe|compact|on|status`
或 `voltui config memory-v5 off|observe|compact|on|status`，并且只认用户级设置。只有明确想为
