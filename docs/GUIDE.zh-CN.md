...
| `Shift+Tab` | 切换 Plan 开/关 | Plan 只改变“先规划”的工作流；内置 writer 仍走当前 Ask/Auto/YOLO 与 Sandbox，MCP writer/destructive 目标在计划批准前保持阻断。 |
...
| Plan | 要求模型先规划——这是 plan-first 工作流，不是全部工具只读。内置 writer 仍遵守当前 Ask/Auto/YOLO 与 Sandbox；已安装 MCP writer、destructive 目标与未信任 reader 在审批前硬阻断；`complete_step` 等显式阶段工具需等到计划批准后。 |
...
所有严格只读子会话都经过同一个构造边界（`RunReadOnlySubAgentWithSession`）：
它把子会话标记为永久只读，并做最终 registry 过滤——移除 writer、destructive MCP
目标、外部自报但未信任的 reader，以及任何会启动或改变 host capability 的工具。
严格只读入口一览：
...
这一层比主 Plan 更严格：Plan 在批准前阻断 MCP writer/destructive 目标、内置 writer
仍走 Permissions/Sandbox，而严格只读子会话根本不暴露 writer。
