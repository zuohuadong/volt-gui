...
| `Shift+Tab` | 切换 Plan 开/关 | Plan 只改变“先规划”的工作流；当前 Ask/Auto/YOLO 与 Sandbox 仍生效。 |
...
| Plan | 要求模型先规划。内置工具、MCP 和代理解析后的 writer 都遵守当前 Ask/Auto/YOLO 与 Sandbox；`complete_step` 等显式阶段工具需等到计划批准后。 |
...
`trusted_read_only_tools`。没有 `readOnlyHint` 的工具仍按写工具处理。计划期间，内置工具、MCP 和
代理解析后的 writer 都进入同一套 Permissions/Sandbox 路径。
