...
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./voltui.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
VoltUI 全局 `<VoltUI home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 VoltUI 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
插件包可通过 `hooks/session-start-codex` 或插件根目录 `CLAUDE.md` 提供该启动上下文；Claude 风格 `.claude/settings.json` command hooks 也会按同名事件映射到 VoltUI hooks。
...
# password_hash = ""           # 用 voltui serve --hash-password --password '...' 生成
...
多数日常设置应写在 `config.toml` 或前文提到的 VoltUI 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 VoltUI 之前设置。项目 `.env` 不是 VoltUI 控制变量的运行时来源。

## Serve Web 前端

`voltui serve` 会用同一个本地 VoltUI 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
`voltui acp` 把 VoltUI 作为 ACP v1 stdio agent 提供给编辑器和其他 host 客户端。
...
远程模块让 VoltUI 在远端主机上运行,并通过你自己的 SSH 连接访问它 —— 即 VS Code
Remote-SSH 式的体验。它在远端主机上引导一个常驻的 headless `voltui serve`,把本地一个
...
`voltui.toml` 无法注入或覆盖远程主机 —— 克隆的仓库永远无法左右 VoltUI 向何处发起 SSH
连接。凭据沿用 provider 惯例:主机只记录环境变量名(`passphrase_env`、`password_env`),其值
存放在 VoltUI 全局 `.env` 中;私钥内容本身从不存储 —— `identity_file` 只是路径。
...
```bash
voltui remote add gpu-box dev@203.0.113.7 --workspace '~/projects/app'
voltui remote import --all              # 导入别名；连接时通过 ssh -G 解析 Include/Match 等规则
voltui remote test gpu-box              # 拨号 + 认证 + 主机密钥确认
voltui remote connect gpu-box --open    # 引导 serve、建隧道、打开 URL
voltui remote serve status gpu-box
voltui remote fs ls gpu-box:'~/projects/app'
```
...
主机密钥会对照你的 OpenSSH `~/.ssh/known_hosts`(只读)以及 VoltUI 托管的
`~/.voltui/remote/known_hosts` 校验。首次见到的密钥会提示 TOFU 确认并记入托管文件;与已记录
密钥冲突的密钥会硬失败并指明出错的行,绝不自动接受。

远端侧状态位于远端主机的 `~/.voltui/remote/`:`serve-<工作区 slug>.json`(pid、绑定的回环
...
Remote SSH 的独立 VoltUI 原生窗口。主窗口持有 SSH 隧道；远程窗口是隔离的轻量外壳，不会恢复
或抢占本地对话会话。
...
常用服务优先使用 **添加模型服务 -> 推荐预设**。VoltUI 可以预填可编辑的自定义 provider：
...
预设路径通常只需要填写服务商 API Key：真实 key 会写入 VoltUI home `.env`，
...
**API 地址** 填写服务端点。默认模式下，VoltUI 会预览并把聊天请求发送到：
...
开启 **完整 URL**。开启后 VoltUI 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
| 上下文窗口 | VoltUI 用于自动清理上下文的 provider 级 token 预算。`0` 表示禁用自动 compaction。 | 按该 provider 的模型上下文上限填写；所选模型规格不同时使用下方的逐模型覆盖。 |

每个已选模型还提供一个可选的 **上下文窗口** 输入框。留空时继承 provider
级设置；填写正整数时只覆盖该模型。这样，同一端点下的长上下文模型不会过早
compaction，短上下文模型也不会在 VoltUI 清理前被服务端拒绝。
...
| 选项 | 作用 |
| --- | --- |
| 自动识别（推荐） | VoltUI 根据模型能力元数据和端点自动选择请求格式。 |
...
| 选项 | 作用 |
| --- | --- |
| Auto（使用服务默认） | 不写 provider 级 `thinking` 覆盖，让 VoltUI 使用 provider/model 默认行为。 |
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择可配置的行后按下新的组合键，VoltUI 会为桌面端保存该绑定。
...
| macOS `Cmd+Z`，Windows/Linux `Ctrl+Z` | 撤销输入框中的最近一次编辑 | 普通键入继续由 WebView 原生历史管理；VoltUI 接管的粘贴、剪切、折叠块和结构化 token 会作为完整事务恢复。 |
...
VoltUI 始终会从工具子进程环境中移除已保存的 provider 与 bot 凭据变量，并自动把
全局凭据 `.env` 加入运行时禁读边界；项目 `.env` 仍保持现有的 workspace 范围行为。
**Windows 说明：**VoltUI 不在 Windows 上提供 OS 级 Bash 沙箱，生效模式固定为
`off`。旧配置即使写了 `bash = "enforce"` 也会解析为 `off`，`voltui doctor`
...
反馈编码质量问题时，可运行 `voltui doctor quality <branch-id-or-path>`（加
...
或 Discussion。它不同于 `voltui doctor session`：后者生成的支持 zip 含完整未脱敏
会话，只能在可信支持渠道分享。
...
voltui doctor capabilities

# 机器可读（stdout 仅为合法 JSON）
voltui doctor capabilities --json

# 指定工作区
voltui doctor capabilities --root /path/to/project

# Live MCP 探测——仅在你明确允许启动第三方服务器时使用
voltui doctor capabilities --live --timeout 5s
```

| 入口 | 用法 |
| --- | --- |
| CLI | 见上方 `voltui doctor capabilities` |
| 桌面端 | **设置 → 诊断** — 刷新、复制脱敏 JSON、可选「包含当前会话运行状态」（只读活动标签 Host，**不**启动 MCP） |
| Agent | `/voltui-guide`（内置 inline Skill）或自然语言描述症状；优先静态 doctor JSON，再问是否 `--live` |
...
MCP 名称按 workspace 解析：项目声明覆盖同名全局安装；项目内部以 `voltui.toml` 高于
...
VoltUI 明确信任已安装 server 会如实描述这些 hint。因此，planner/只读 subagent 的过滤是
面向可信 server 的工作流边界，不是针对恶意 MCP server 的隔离边界；显式 deny 与进程沙箱
仍由 host 控制。

旧的 `trusted_read_only_tools`、`default_tools_approval_mode`、
`tools.<raw>.approval_mode` 与 `approvals_reviewer` 字段在加载旧文件时会被忽略，并在 VoltUI
下次保存该 MCP 条目时自动移除。
...
[能力诊断](./CAPABILITY_DIAGNOSTICS.zh-CN.md)
（`voltui doctor capabilities` 或 **设置 → 诊断**）。
...
交互式 `voltui` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/work-mode`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
内置 **Skill**（如 `/init`、`/explore`、`/test`、`/voltui-guide`）也会出现在斜杠菜单，
并可通过 `run_skill` 调用（正文按需加载；只有索引行进入缓存稳定前缀）。配置或能力排障时
用 `/voltui-guide`，它会引导运行 `voltui doctor capabilities`（见
[能力诊断](./CAPABILITY_DIAGNOSTICS.zh-CN.md)）。
...
另一端发现。交互式聊天里使用 `/<name> <任务>` 调用；VoltUI 会启动隔离子智能体，
父会话只保留任务和最终答案。

Headless CLI 提供显式管理和运行命令，同时不改变普通 `voltui run` 的任务语义：

```bash
voltui subagent list
voltui subagent create reviewer --description "审查改动" --prompt-file reviewer.md --tools read_file,grep,bash
voltui subagent edit reviewer --effort high --model deepseek-pro
voltui subagent try reviewer "审查当前 diff"   # 始终只读
voltui subagent run reviewer "审查并修复当前 diff"
voltui subagent delete reviewer --yes
```
...
`voltui setup` 现在统一管理 provider、模型列表、凭据、连接测试和默认模型；所有修改
会暂存到“保存并退出”，并同步维护桌面端 provider access。完整用法见
[CLI 命令参考](./CLI.zh-CN.md#配置供应商)。若要让两个模型协同（执行器 + 规划器，
各自独立、缓存稳定的 session），向导后手动在 `voltui.toml` 加一行即可：
...
VoltUI 会自动管理正常执行：活跃 Todo 连续 8 个工具调用轮次没有新的完成项、唯一读取、
...
| `voltui review`（CLI） | 只读评审 diff 或分支 |
...
`voltui run --profile delivery "修复并验证这个 bug"`。Economy（轻量）初始只带 9 个工具：
...
`Shift+Tab` 切换到 Plan。VoltUI 先生成计划，待用户批准后工作流才切换到实施；规划期间的
...
`voltui config reasoning-language auto|zh|en`。只有明确想为
