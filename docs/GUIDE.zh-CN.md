...
[agent]
max_steps = 32                   # 仅用户/全局；执行器工具调用轮数；0 表示不限制总轮次
...
`max_steps` 限制执行器；`planner_max_steps` 只限制规划器，两者都可设为 `0` 表示不设总轮次上限。
此外，只要执行器仍有活跃 Todo，当前项连续 16 个工具调用轮次未完成时就会暂停；已保存的工作可在
下一轮用户消息中继续。
