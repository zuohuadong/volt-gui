...
- 本地目录，目录内需要包含 `voltui-plugin.json`、
...
- 既未声明 `args` 也未声明 `shell` 的已有原生 Hook 继续使用 VoltUI
...
VoltUI 的对应实现，并不代表导入 Hook 的每一种运行时决策都被遵守。
...
- `agents/*.md` 映射为插件所属、需要手动调用的子代理配置。Claude 模型别名会继承
  当前 VoltUI 模型；内联 `tools` 列表会转换为 VoltUI 工具名，并支持
...
- `.claude/settings.json` 和 `hooks/hooks.json` 里的 command hooks 会按同名事件映射。
  ...
  `tool_name` 会在 VoltUI 与 Claude 的工具名之间互译（`bash` ↔ `Bash`、
  `write_file` ↔ `Write` 等），因此 `"Bash"` 这类 matcher 能正确触发；VoltUI 里所有会
  ...
  `NotebookEdit` 则会从 VoltUI 接受的别名补 `new_source`，删除或空单元格操作补空串。
  ...
  `voltui plugin doctor <名称>` 和 `voltui doctor capabilities` 会在 Hook 首次触发前
  ...
  字段也不会被求值。声明其中之一、声明 `Stop`/`SubagentStop` hook（VoltUI 中
  不能阻止本轮结束），或 matcher 覆盖三种无法无损表达的输入时，插件都会报告
  部分兼容并附具体警告：`WebFetch.prompt`、VoltUI 以 `cell_number` 调用时的
  `NotebookEdit.cell_id`，以及 VoltUI `wait` 同时覆盖多个/全部任务时的
