...
Inside `voltui`, use the slash command:
...
`voltui.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
voltui run "explain this module"
```
...
```text
./voltui.toml > user config.toml > built-in defaults
```
...
VoltUI adds the same small transient `<reasoning-language>` block for that
...
When set to `zh` or `en`, VoltUI always adds a small transient
