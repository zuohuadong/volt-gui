...
└── internal/
    ...
    │   └── bootstrap/       # detached `voltui serve` bootstrap over SSH
...
- Configuration provenance is runtime metadata and determines persistence scope.
  ...
  `voltui.toml` and `.mcp.json` entries remain in their owning project file.
  ...
  `voltui.toml` overrides `.mcp.json`. Editing writes to the effective entry's
...
- Installation is the trust decision for tool metadata. VoltUI assumes an
...
- `voltui subagent try <name> ... <task>` uses the read-only Skill runner;
- `voltui subagent run <name> ... <task>` uses the normal permission and
  sandbox path; and
- ordinary `Controller.Run` / `voltui run` remains unchanged and does not
...
Fields marked user/global only are not overridden by project `voltui.toml`.
...
without importing provider keys or VoltUI control variables.
...
`network = true`.
**Windows status:** VoltUI does not ship an OS-level Bash sandbox on Windows.
The effective mode is fixed to `off`; an older config containing
`bash = "enforce"` remains readable but resolves to `off`, `voltui doctor`
