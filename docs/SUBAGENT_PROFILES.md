...
with `runAs: subagent`: VoltUI starts an isolated child agent, gives it the
...
```bash
voltui subagent create reviewer \
...
`.voltui/skills/<name>/SKILL.md`; global profiles are stored under the
VoltUI home Skill directory described in
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  voltui subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. VoltUI refuses a name
...
voltui subagent try reviewer "review the current diff"
...
git diff | voltui subagent run reviewer --max-steps 20
```
...
apply. Ordinary `voltui run` remains a plain one-shot task entry point and
does not implicitly interpret `/<profile>` syntax.

## Manage profiles

```text
voltui subagent list [--dir PATH]
voltui subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
voltui subagent edit <name> [--description TEXT]
  [--prompt TEXT | --prompt-file PATH] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
voltui subagent delete <name> --yes [--dir PATH]
voltui subagent try <name> [--model REF] [--max-steps N] [--dir PATH] <task>
voltui subagent run <name> [--model REF] [--max-steps N] [--dir PATH] <task>
```
...
```bash
voltui subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `voltui subagent create` share
...
`voltui subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when VoltUI resolves the effective model.
