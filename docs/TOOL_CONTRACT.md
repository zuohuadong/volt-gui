...
`ask`, `bash`, `bash_output`, `calculate`, `code_index`, `complete_step`, `connect_tool_source`, `edit_file`, `forget`, `glob`, `grep`, `history`, `kill_shell`, `list_sessions`, `ls`, `memory`, `move_file`, `multi_edit`, `read_file`, `read_session`, `remember`, `slash_command`, `todo_write`, `wait`, `write_file`.
