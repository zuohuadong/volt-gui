...
- Replace the current chat-first desktop shell with a Svelte 5 workbench that
...
6. **Resource surfaces**: migrate settings and operations panels to typed
   resources.
7. ~~**Replace legacy shell**: switch Wails build commands to the Svelte frontend~~ (done)
   only after parity gates pass.
8. ~~**Remove obsolete frontend code**: remove legacy dependencies and dead components~~ (done)
