...
// the file tools when a target is a VoltUI-managed config file outside the
...
type Options struct {
	...
	// ConfigWriteApprover confirms file-tool writes to VoltUI-managed config
...
// parallelised. The second return carries image results aligned with the text
// results, keeping binary payloads out of truncateToolOutput.
...
// (without the "· " prefix) when the output was head+tailed. Images stay
// outside text so truncation cannot corrupt a base64 payload.
...
func (a *Agent) executeOne(ctx context.Context, call provider.ToolCall) toolOutcome {
	...
	var images []string
	var err error
	if imageTool, ok := t.(tool.ImageTool); ok {
		result, images, err = imageTool.ExecuteWithImages(cctx, json.RawMessage(call.Arguments))
	} else {
