...
func TestRememberTrustedIntranetWritesExactUserGlobalRuleAndMapsRuntimePolicy(t *testing.T) {
	...
	req := tool.TrustedIntranetRequest{URL: "https://lims.volt.org/workshop", Host: "LIMS.volt.ORG.", IP: "192.168.1.14", Port: 443}
	...
	if !cfg.TrustedIntranetAllows("lims.volt.org", "192.168.1.14", 443) {
		t.Fatalf("saved config does not allow exact target: %#v", cfg.Network.TrustedIntranet)
	}
	if cfg.TrustedIntranetAllows("lims.volt.org", "192.168.1.15", 443) || cfg.TrustedIntranetAllows("lims.volt.org", "192.168.1.14", 80) {
	...
	if !policy.Allows("lims.volt.org", net.ParseIP("192.168.1.14"), 443) {
