...
func TestRecoveryPauseTurnDoneIsInformational(t *testing.T) {
	...
	const backendFallback = "Automatic retries paused. VoltUI stopped repeated attempts and kept completed work. Send \"continue\" to start a fresh attempt, or add instructions to change direction."
	tests := []struct {
		lang string
		want string
	}{
		{
			lang: "en",
			want: "Automatic retries paused. VoltUI stopped repeated attempts and kept completed work. Send “Continue” to start a fresh attempt, or add instructions to change direction.",
		},
		{
			lang: "zh",
			want: "已暂停自动重试。VoltUI 已停止重复尝试，并保留已完成的工作。发送“继续”即可开始新一轮，也可以补充要求来调整方向。",
		},
		{
			lang: "zh-TW",
			want: "已暫停自動重試。VoltUI 已停止重複嘗試，並保留已完成的工作。傳送「繼續」即可開始新一輪，也可以補充要求來調整方向。",
		},
	}
	for _, tt := range tests {
