...
// "volt" (internal gateway), "deepseek" (flash + pro), and "mimo" (pro + flash),
...
func TestGroupByFamily(t *testing.T) {
	...
	if got := order; !reflect.DeepEqual(got, []string{"volt", "deepseek", "mimo-pro", "mimo-flash"}) {
		t.Fatalf("family order = %v, want [volt deepseek mimo-pro mimo-flash]", got)
	}
	if got := members["volt"]; !reflect.DeepEqual(got, []int{0, 1, 2, 3}) {
		t.Errorf("volt members = %v, want [0 1 2 3]", got)
	}
	...
	if info["volt"].name != "西谷内网" || info["deepseek"].name != "DeepSeek" || info["mimo-pro"].name != "mimo-pro" || info["mimo-flash"].name != "mimo-flash" {
		t.Errorf("display names = %q / %q / %q / %q", info["volt"].name, info["deepseek"].name, info["mimo-pro"].name, info["mimo-flash"].name)
	}
}
