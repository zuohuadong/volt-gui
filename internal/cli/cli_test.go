...
func TestGroupByFamily(t *testing.T) {
	...
	if got := order; !reflect.DeepEqual(got, []string{"volt", "deepseek", "mimo"}) {
		t.Fatalf("family order = %v, want [volt deepseek mimo]", got)
	}
	...
	if info["volt"].name != "西谷内网" || info["deepseek"].name != "DeepSeek" || info["mimo"].name != "MiMo (Xiaomi)" {
		t.Errorf("display names = %q / %q / %q", info["volt"].name, info["deepseek"].name, info["mimo"].name)
	}
}
