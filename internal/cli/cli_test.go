...
// TestGroupByFamily verifies the wizard groups default presets by provider
// family, preserving the order each family first appears in.
func TestGroupByFamily(t *testing.T) {
	order, members, info := groupByFamily(config.Default().Providers)

	if got := order; !reflect.DeepEqual(got, []string{"deepseek", "mimo-pro", "mimo-flash"}) {
		t.Fatalf("family order = %v, want [deepseek mimo-pro mimo-flash]", got)
	}
