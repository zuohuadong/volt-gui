...
func TestApplyUserConfigUpgradesOnStartupMigratesLegacyBundledvoltStep(t *testing.T) {
	...
	cfg.Providers = []ProviderEntry{{
		Name: legacyvoltStepProvider, Kind: "openai", BaseURL: baseURL,
		Model: legacyvoltStepModel, APIKeyEnv: "volt_API_KEY", ContextWindow: 131_072, NoProxy: true,
	}}
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
...
func TestApplyUserConfigUpgradesOnStartupDeduplicatesLegacyStep(t *testing.T) {
	...
	cfg := Default()
	cfg.ConfigVersion = 5
	cfg.DefaultModel = legacyvoltStepProvider
	cfg.Desktop.ProviderAccess = []string{legacyvoltStepProvider, "glm-5.2", "custom"}
...
func TestApplyUserConfigUpgradesOnStartupPreservesCustomVLMDefaultBeforeV7(t *testing.T) {
	...
	cfg.DefaultModel = "vlm"
	cfg.Providers = []ProviderEntry{
		{Name: "xllm", Kind: "openai", BaseURL: baseURL, Model: "glm-5.2/glm-5.2", APIKeyEnv: "volt_API_KEY"},
...
func TestApplyUserConfigUpgradesOnStartupPreservesCustomStepProvider(t *testing.T) {
	...
	cfg.DefaultModel = legacyvoltStepProvider
	cfg.Providers = append(cfg.Providers, ProviderEntry{
		Name: legacyvoltStepProvider, Kind: "openai", BaseURL: "https://custom.example/v1",
		Model: legacyvoltStepModel, APIKeyEnv: "CUSTOM_STEP_KEY", ContextWindow: 200_000,
	})
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
	...
	if got.DefaultModel != legacyvoltStepProvider {
...
func TestApplyUserConfigUpgradesOnStartupMigratesLegacyStepPreservingCustomGLM(t *testing.T) {
	...
	cfg.Providers = []ProviderEntry{
		...
		{Name: legacyvoltStepProvider, Kind: "openai", BaseURL: baseURL, Model: legacyvoltStepModel, APIKeyEnv: "volt_API_KEY", ContextWindow: 131_072, NoProxy: true},
	}
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
...
func TestApplyUserConfigUpgradesOnStartupMigratesLegacyBundledvoltGLM(t *testing.T) {
	const baseURL = "http://gateway.internal.test:9010/v1"
	useBundledvoltGateway(t, baseURL)
	...
	cfg.Providers = []ProviderEntry{{
		...
		Model: legacyvoltGLMModel, APIKeyEnv: "volt_API_KEY", ContextWindow: 131_072, NoProxy: true,
	}}
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
