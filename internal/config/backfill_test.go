...
func useBundledXiguGateway(t *testing.T, baseURL string) {
	t.Helper()
	testRoot := t.TempDir()
...
func TestApplyUserConfigUpgradesOnStartupPreservesCustomStepProvider(t *testing.T) {
	useBundledXiguGateway(t, "http://gateway.internal.test:9010/v1")
...
func TestApplyUserConfigUpgradesOnStartupPreservesCustomVLMNameCollision(t *testing.T) {
	...
	cfg.Providers = []ProviderEntry{
		{Name: "xllm", DisplayName: "客户文本模型", Kind: "openai", BaseURL: baseURL, Model: "glm-5.2/glm-5.2", APIKeyEnv: "XIGU_API_KEY", ContextWindow: 131_072, NoProxy: true},
		{Name: "vlm", Kind: "openai", BaseURL: "https://custom.example/v1", Model: "custom-vision", APIKeyEnv: "CUSTOM_VLM_KEY", Vision: true},
	}
	if err := cfg.SaveTo(path); err != nil {
		t.Fatalf("SaveTo: %v", err)
