...
func TestDefaultUsesBundledXiguGateway(t *testing.T) {
	...
	for name, expected := range want {
		...
		if entry.BaseURL != baseURL || entry.Model != expected.model || entry.DisplayLabel() != expected.label || entry.APIKeyEnv != "XIGU_API_KEY" || entry.Vision != expected.vision || !entry.NoProxy {
