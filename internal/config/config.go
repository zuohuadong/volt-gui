...
func bundledvoltProviderDefaults() (string, []ProviderEntry) {
	...
	return "xllm", []ProviderEntry{
		...
		{Name: "vlm", DisplayName: "vlm", Kind: "openai", BaseURL: baseURL, Model: "step-3.7-flash/step-3.7-flash", APIKeyEnv: "volt_API_KEY", ContextWindow: 262_144, Vision: true, ToolCalling: &toolCalling, NoProxy: true},
	}
}

// IsBundledvoltCatalogProvider reports whether name still uses one of the
...
func (c *Config) IsBundledvoltCatalogProvider(name string) bool {
	if c == nil {
		return false
	}
	...
	for _, canonical := range bundled {
		if sameBundledvoltRoute(*configured, canonical) {
			return true
		}
	}
	return false
