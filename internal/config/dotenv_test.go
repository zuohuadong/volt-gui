...
// reasonix-owned credentials file under VoltUI home — is loaded from any
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "VoltUI credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want VoltUI credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresVoltUIStateHome(t *testing.T) {
	home := t.TempDir()
	state := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms VoltUI-owned global
