...
func TestTrustedIntranetSiteAddMatchRemoveAndRender(t *testing.T) {
	cfg := Default()
	changed, err := cfg.AddTrustedIntranetSite("LIMS.volt.ORG.", "192.168.1.14", 443)
	if err != nil {
	...
	if got := cfg.Network.TrustedIntranet.Sites; len(got) != 1 || got[0].Host != "lims.volt.org" || len(got[0].CIDRs) != 1 || got[0].CIDRs[0] != "192.168.1.14/32" || len(got[0].Ports) != 1 || got[0].Ports[0] != 443 {
	...
	for _, tc := range []struct {
		...
		{"lims.volt.org", "192.168.1.14", 443, true},
		{"LIMS.volt.ORG.", "192.168.1.14", 443, true},
		{"lims.volt.org", "192.168.1.15", 443, false},
		{"lims.volt.org", "192.168.1.14", 80, false},
		{"other.volt.org", "192.168.1.14", 443, false},
	} {
	...
	if changed, err := cfg.AddTrustedIntranetSite("lims.volt.org", "192.168.1.14", 443); err != nil || changed {
