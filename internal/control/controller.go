...
// approval when a file tool targets a VoltUI-managed config file outside the
...
// managedConfigWriteApprover routes a file tool's VoltUI-managed config write
...
func (c *Controller) requestApprovalDecisionWithOptions(ctx context.Context, tool, subject string, args json.RawMessage, reason string, opts approvalDecisionOptions) (approvalReply, error) {
	...
	// runs synchronously and before the dialog is shown. Native VoltUI
	...
	// sandbox escapes, VoltUI config writes, memory remember/forget, and
