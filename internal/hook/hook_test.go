...
func TestNormalizeCommandRepairsOnlyPowerShellFileEscapedQuotes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			command: `pwsh.exe -NoProfile -NonInteractive -File \"C:\Program Files\VoltUI Hooks\archive attachments.ps1\"`,
			want:    `pwsh.exe -NoProfile -NonInteractive -File "C:\Program Files\VoltUI Hooks\archive attachments.ps1"`,
		},
		{
			name:    "doubly escaped copied quotes",
			command: `pwsh -File \\\"C:\Program Files\VoltUI Hooks\archive attachments.ps1\\\" \"arg with spaces\"`,
			want:    `pwsh -File "C:\Program Files\VoltUI Hooks\archive attachments.ps1" "arg with spaces"`,
		},
		{
		...
		},
		{
			...
			command: `powershell -NoProfile -File "C:\Program Files\VoltUI Hooks\archive attachments.ps1"`,
			want:    `powershell -NoProfile -File "C:\Program Files\VoltUI Hooks\archive attachments.ps1"`,
		},
		{
...
func TestLoadNormalizesPowerShellFileEscapedQuotes(t *testing.T) {
	home := t.TempDir()
	bad := `powershell -File \"C:\Program Files\VoltUI Hooks\archive attachments.ps1\"`
	want := `powershell -File "C:\Program Files\VoltUI Hooks\archive attachments.ps1"`
...
func TestRepairablePowerShellFileArgs(t *testing.T) {
	command := `powershell -NoProfile -NonInteractive -File \"C:\Program Files\VoltUI Hooks\archive attachments.ps1\" -Mode \"startup\"`
	...
	wantArgs := []string{"-NoProfile", "-NonInteractive", "-File", `C:\Program Files\VoltUI Hooks\archive attachments.ps1`, "-Mode", "startup"}
...
func TestExpandPluginRootSupportsClaudeVoltUIAndCmdAliases(t *testing.T) {
	root := `C:\Program Files\VoltUI\plugins\impeccable`
...
func TestWindowsBatchArgvCommandLineSupportsNativePluginHooks(t *testing.T) {
	got, ok := windowsBatchArgvCommandLine(
		`C:\Program Files\VoltUI\plugins\example/hooks/run-hook.cmd`,
		[]string{"session-start", "argument with spaces"},
	)
	if !ok {
		t.Fatal("native plugin batch command was not recognized")
	}
	want := `cmd.exe /d /s /c ""C:\Program Files\VoltUI\plugins\example\hooks\run-hook.cmd" session-start "argument with spaces""`
	if got != want {
...
func TestMatchesToolTranslatesClaudeToolNames(t *testing.T) {
	...
	if !MatchesTool(claude("Bash"), "bash") {
		t.Error(`Claude matcher "Bash" should match VoltUI tool "bash"`)
	}
	if !MatchesTool(claude("Write|Edit"), "write_file") {
		t.Error(`Claude matcher "Write|Edit" should match VoltUI tool "write_file"`)
	}
	if !MatchesTool(claude("Write|Edit"), "edit_file") {
		t.Error(`Claude matcher "Write|Edit" should match VoltUI tool "edit_file"`)
	}
	if MatchesTool(claude("Bash"), "write_file") {
		t.Error(`Claude matcher "Bash" must not match VoltUI tool "write_file"`)
	}
	// A native (non-Claude) hook's matcher stays in VoltUI's own vocabulary.
	...
	// using either name must still fire against VoltUI's "task" tool.
	if !MatchesTool(claude("Agent"), "task") {
		t.Error(`Claude matcher "Agent" (current name) should match VoltUI tool "task"`)
	}
	if !MatchesTool(claude("Task"), "task") {
		t.Error(`Claude matcher "Task" (legacy alias) should still match VoltUI tool "task"`)
	}
	if !MatchesTool(claude("AskUserQuestion"), "ask") {
		t.Error(`Claude matcher "AskUserQuestion" should match VoltUI tool "ask"`)
	}
	for _, name := range []string{"bash_output", "wait"} {
		if !MatchesTool(claude("TaskOutput"), name) || !MatchesTool(claude("BashOutput"), name) {
			t.Errorf(`current "TaskOutput" and legacy "BashOutput" matchers should match VoltUI tool %q`, name)
		}
	}
	if !MatchesTool(claude("TaskStop"), "kill_shell") || !MatchesTool(claude("KillShell"), "kill_shell") {
		t.Error(`current "TaskStop" and legacy "KillShell" matchers should match VoltUI tool "kill_shell"`)
	}
}
...
func TestClaudeFacingToolNameUsesCurrentNames(t *testing.T) {
	...
	for _, name := range []string{"task", "read_only_task", "parallel_tasks", "explore", "research", "review", "security_review"} {
		...
		if !MatchesTool(claude, name) {
			t.Errorf(`Claude matcher "Agent" should match VoltUI tool %q`, name)
		}
		legacy := ResolvedHook{HookConfig: HookConfig{Match: "Task", PayloadFormat: "claude"}, Event: PreToolUse}
		if !MatchesTool(legacy, name) {
			t.Errorf(`legacy Claude matcher "Task" should still match VoltUI tool %q`, name)
		}
	}
}
...
// contract ("file_path must be absolute"): a relative VoltUI path resolves
...
func TestDecideOutcome(t *testing.T) {
	cases := []struct {
		...
		// same way PreToolUse does; native VoltUI PermissionRequest hooks
...
func TestRunClaudePayloadAndDirectArgs(t *testing.T) {
	...
	if payload["tool_name"] != "Bash" {
		t.Fatalf("Claude payload tool_name = %v, want the Claude vocabulary name Bash for VoltUI tool bash", payload["tool_name"])
	}
...
// VoltUI write_file call and sees the absolute target path Claude's
...
func TestRunClaudeWriteFileGuardFiresAndSeesFilePath(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Write" did not fire for VoltUI tool "write_file"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
	...
	if _, hasPath := toolInput["path"]; hasPath {
		t.Fatalf("tool_input still has VoltUI's \"path\" key: %#v", toolInput)
	}
}

// TestRunClaudeAgentGuardFiresAndSeesRequiredFields covers the full matcher to
// stdin path for a dedicated VoltUI subagent wrapper. Claude Agent requires
...
func TestRunClaudeAgentGuardFiresAndSeesRequiredFields(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Agent" did not fire for VoltUI tool "security_review"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
