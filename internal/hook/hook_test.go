...
func TestWindowsExtensionlessPOSIXPluginHookUsesBash(t *testing.T) {
	...
	config := pluginHookExecutionConfigForPlatform(pluginpkg.Hook{
		Command: "hooks/session-start-codex",
	}, root, "windows")
...
func TestWindowsImplicitPluginShellUsesPOSIXPluginRoot(t *testing.T) {
	root := `C:\Users\Test User\AppData\Roaming\reasonix\plugins\superpowers`
	config := pluginHookExecutionConfigForPlatform(pluginpkg.Hook{
		Command:      `"${CLAUDE_PLUGIN_ROOT}/hooks/start.sh"`,
		ShellCommand: true,
	}, root, "windows")
	want := `"C:/Users/Test User/AppData/Roaming/reasonix/plugins/superpowers/hooks/start.sh"`
	if config.ExecutionMode != ExecutionShell || config.Shell != "" {
		t.Fatalf("implicit shell contract = mode %q shell %q", config.ExecutionMode, config.Shell)
	}
	if config.Command != want {
		t.Fatalf("implicit shell command = %q, want POSIX plugin root %q", config.Command, want)
	}
}

func TestWindowsImplicitPluginPOSIXScriptRequiresBash(t *testing.T) {
	root := filepath.Join(t.TempDir(), "plugin root")
	writeHookTestFile(t, filepath.Join(root, "hooks", "start.sh"), "#!/usr/bin/env bash\necho ok\n")
	config := pluginHookExecutionConfigForPlatform(pluginpkg.Hook{
		Command:      "hooks/start.sh",
		ShellCommand: true,
	}, root, "windows")
	if config.Shell != "bash" || config.ExecutionMode != ExecutionShell {
		t.Fatalf("implicit POSIX hook contract = mode %q shell %q, want explicit Bash", config.ExecutionMode, config.Shell)
	}
	if want := bashSingleQuote(filepath.ToSlash(filepath.Join(root, "hooks", "start.sh"))); config.Command != want {
		t.Fatalf("implicit POSIX hook command = %q, want %q", config.Command, want)
	}
}
