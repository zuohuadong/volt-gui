...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `voltui`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `voltui`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `voltui setup`。",
	...
	NoSessionToResume: "没有可恢复的会话 — 用 `voltui` 开一个新的",
	...
	PlanModeBashTrustReason:                "这条 bash 命令不在 VoltUI 内置只读集合中。只有在确认这个精确前缀用于计划和研究时是只读的，才应批准。自动/YOLO 审批不能回答这个信任提示。",
	...
	ApprovalToolLabelConfigWrite:           "VoltUI 配置写入审批",
	ConfigWriteSubjectPrefix:               "写入 VoltUI 配置：",
	ConfigWriteReason:                      "这次写入的目标是工作区之外的 VoltUI 托管配置文件。它可以改变后续会话的模型服务商、沙箱规则、权限和 MCP 服务器，因此需要你的明确批准。",
	ConfigWriteDeclined:                    "用户拒绝了这次 VoltUI 配置写入；不要重试，请询问用户希望如何继续。",
	...
	OutputStyleHint:    "在 voltui.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 voltui.toml 加一个 [[plugins]] 条目",
	...
	MissingKeyIntro:       "voltui.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API key（缺失或未设置）。请在 .env 中配置密钥，或运行 `voltui setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `voltui setup`。",
...
用法：
  ...
  voltui config memory-v5 [off|observe|compact|on|status]  配置 Memory v5
  voltui config reasoning-language [auto|zh|en]        配置可见思考语言
  voltui mcp <add|remove|list|import>                 管理 voltui.toml 里的 MCP 服务器
  voltui init                                         查看如何生成项目记忆（AGENTS.md）
  voltui doctor [--json]                              输出脱敏的本地诊断信息
  voltui bot start|doctor|weixin-login                多渠道 IM bot 网关
  voltui upgrade [--check] [--force]                   自更新到最新版本（也可用：voltui update）
  voltui version
  voltui help
...
配置：
  优先级：flag > ./voltui.toml > ~/.voltui/config.toml > 内置默认值
