...
var Chinese = Messages{
	...
	RecoveryPaused:      "已暂停自动重试。VoltUI 已停止重复尝试，并保留已完成的工作。发送“继续”即可开始新一轮，也可以补充要求来调整方向。",
	...
	ListHooksNone:       "无生效 hooks — 在 .voltui/settings.json（项目）或 <VoltUI home>/settings.json（全局）配置",
...
配置：
  优先级：flag > ./reasonix.toml > <VoltUI home>/config.toml > 内置默认值
