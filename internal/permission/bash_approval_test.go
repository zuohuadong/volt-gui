...
func TestPolicyRawBashDenyMatchesGlobMetaInSubject(t *testing.T) {
	policy := New("allow", nil, nil, []string{"Bash(git*)"})
	args := []byte(`{"command":"git* status"}`)
	if got := policy.Decide("bash", false, args); got != Deny {
		t.Fatalf("deny with subject glob meta = %v, want Deny", got)
	}
	if !policy.ExplicitlyDenies("bash", args) {
		t.Fatal("ExplicitlyDenies missed subject glob meta")
	}
}
