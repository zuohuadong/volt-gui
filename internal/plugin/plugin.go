...
type Spec struct {
	...
	// stdio servers keep inheriting VoltUI's cwd while still receiving the
...
// capabilities VoltUI advertises (currently ping and roots/list).
...
type StartPolicy struct {
	...
	// write MCP stats or schema cache files under VoltUI home.
...
// toolName builds VoltUI's canonical model-visible name
...
func (t *remoteTool) ExecuteWithImages(ctx context.Context, args json.RawMessage) (string, []string, error) {
	...
	if tool.HasNonDestructiveMCPExecutionIntent(ctx) {
		...
		if !t.MCPServerAuthorized() || destructive {
			return "", nil, fmt.Errorf("MCP server %q changed the authorization or destructive classification for tool %q; the call was blocked before dispatch — retry so VoltUI can re-apply the current Planner MCP safety boundary", t.client.name, t.rawName)
		}
	}
