...
func (c *client) buildResponsesRequest(req provider.Request) responsesRequest {
	...
	for _, m := range src {
		...
		case provider.RoleUser:
			input = append(input, responsesMessageItem{
				Role:    string(provider.RoleUser),
				Content: responsesInputContent(m.Content, m.Images, c.vision && len(m.Images) > 0, c.visionDetail),
			})
		case provider.RoleAssistant:
			...
			for _, tc := range m.ToolCalls {
				input = append(input, responsesFunctionCallItem{Type: "function_call", CallID: tc.ID, Name: tc.Name, Arguments: tc.Arguments})
			}
		case provider.RoleTool:
			input = append(input, responsesFunctionCallOutputItem{Type: "function_call_output", CallID: m.ToolCallID, Output: m.Content})
			if c.vision {
				pendingToolImages = append(pendingToolImages, m.Images...)
			}
		}
	...
	for _, t := range req.Tools {
		...
		tools = append(tools, responsesTool{Type: "function", Name: t.Name, Description: t.Description, Parameters: parameters})
	}
...
func reservedResponsesBodyField(name string) bool {
	switch strings.ToLower(strings.TrimSpace(name)) {
	case "model", "input", "instructions", "tools", "stream", "temperature", "max_output_tokens", "reasoning", "store":
		return true
	default:
		return false
	}
