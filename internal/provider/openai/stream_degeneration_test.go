...
func TestReadStreamStopsRepeatedCJKAcrossFrames(t *testing.T) {
	...
	emitted, err := (&client{name: "gateway", model: "glm-5.2/glm-5.2"}).readStream(context.Background(), response, chunks)
...
func TestReadStreamDoesNotGuardUnrelatedModels(t *testing.T) {
	stream := "data: {\"choices\":[{\"delta\":{\"content\":\"" + strings.Repeat("推", 40) + "\"}}]}\n\n"
	stream += "data: [DONE]\n\n"
	response := &http.Response{Body: io.NopCloser(strings.NewReader(stream))}
	chunks := make(chan provider.Chunk, 8)
	if _, err := (&client{name: "custom", model: "custom-model"}).readStream(context.Background(), response, chunks); err != nil {
		t.Fatalf("unrelated model stream was guarded: %v", err)
	}
}
...
func TestReadStreamReasoningLimitDoesNotApplyToUnrelatedModels(t *testing.T) {
	...
	chunks := make(chan provider.Chunk, 8)
	if _, err := (&client{name: "custom", model: "custom-model"}).readStream(context.Background(), response, chunks); err != nil {
		t.Fatalf("unrelated model hit the GLM/Step reasoning limit: %v", err)
	}
}
