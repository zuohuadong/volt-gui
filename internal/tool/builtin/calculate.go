...
func (calculate) SnipHint() tool.SnipHint {
	return tool.SnipHint{Head: 8, Tail: 2, HeadChars: 2000, TailChars: 200}
}

func (calculate) Execute(_ context.Context, args json.RawMessage) (string, error) {
	requestArgs, err := decodeCalculateArgs(args)
	if err != nil {
		return "", err
	}
	calculationResult, err := calculation.Evaluate(calculation.Request{
		Expression: requestArgs.Expression,
		Mode:       requestArgs.Mode,
		Scale:      requestArgs.Scale,
		Rounding:   requestArgs.Rounding,
		Currency:   requestArgs.Currency,
	})
	if err != nil {
		return "", err
	}
	encodedResult, err := json.Marshal(calculationResult)
	if err != nil {
...
func decodeCalculateArgs(args json.RawMessage) (calculateArgs, error) {
	if err := rejectNullCalculateArgs(args); err != nil {
		return calculateArgs{}, err
	}
	var requestArgs calculateArgs
	decoder := json.NewDecoder(bytes.NewReader(args))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&requestArgs); err != nil {
		return calculateArgs{}, fmt.Errorf("invalid args: %w", err)
	}
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		if err == nil {
			return calculateArgs{}, fmt.Errorf("invalid args: multiple JSON values")
		}
		return calculateArgs{}, fmt.Errorf("invalid args: %w", err)
	}
...
func rejectNullCalculateArgs(args json.RawMessage) error {
	var rawFields map[string]json.RawMessage
	if err := json.Unmarshal(args, &rawFields); err != nil {
		return fmt.Errorf("invalid args: %w", err)
	}
	for name, encodedValue := range rawFields {
		if bytes.Equal(bytes.TrimSpace(encodedValue), []byte("null")) {
