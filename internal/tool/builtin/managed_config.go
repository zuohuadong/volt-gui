...
// ManagedConfigPaths is the set of VoltUI-owned configuration FILES a file
...
func (m ManagedConfigPaths) approve(ctx context.Context, target string) error {
	...
	if !ok {
		return fmt.Errorf("path %q is a VoltUI-managed config file outside the writable roots; writing it requires interactive user approval, which this session cannot provide. "+
	...
	if !allow {
		if strings.TrimSpace(reason) == "" {
			reason = "the user declined this VoltUI config write — do not retry it; ask how they would like to proceed."
		}
