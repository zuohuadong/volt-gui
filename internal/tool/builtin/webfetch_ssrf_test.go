...
func TestWebFetchPrivateAddressApprovalContinuesSameCallAndDoesNotLeakToNextCall(t *testing.T) {
	...
	approver := &trustedIntranetApproverStub{allow: func(req tool.TrustedIntranetRequest) bool {
		return req.Host == "lims.volt.org" && req.IP == "192.168.1.14" && req.Port == port
	}}
	wf := webFetch{
	...
	args, _ := json.Marshal(map[string]any{"url": "http://lims.volt.org:" + portText + "/start"})
...
func TestWebFetchPrivateAddressPolicyMatchSkipsApproval(t *testing.T) {
	...
	wf := webFetch{
		trustedIntranet: TrustedIntranetPolicy{Enabled: true, Sites: []TrustedIntranetSite{{Host: "lims.volt.org", CIDRs: []string{"192.168.1.14/32"}, Ports: []int{port}}}},
		lookupIP: func(context.Context, string) ([]net.IPAddr, error) {
			return []net.IPAddr{{IP: net.ParseIP("192.168.1.14")}}, nil
		},
		dialContext: func(ctx context.Context, network, _ string) (net.Conn, error) {
	...
	ctx := tool.WithTrustedIntranetApprover(context.Background(), approver)
	args, _ := json.Marshal(map[string]any{"url": "http://lims.volt.org:" + portText})
