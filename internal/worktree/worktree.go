...
// Delivery sessions. Worktrees live under VoltUI-managed state, never inside
...
func Create(ctx context.Context, workspaceRoot, managedRoot string) (Result, error) {
	...
	if managedRoot == "" {
		return Result{}, errors.New("VoltUI worktree storage is unavailable")
	}
	if err := os.MkdirAll(managedRoot, 0o700); err != nil {
		return Result{}, fmt.Errorf("create VoltUI worktree storage: %w", err)
	}
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		branch := fmt.Sprintf("voltui/delivery-%s-%s", time.Now().Format("20060102-150405"), id)
...
// IsManagedPath reports whether path belongs to VoltUI's durable worktree
