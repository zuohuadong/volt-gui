...
describe('DSH Core Engine Tests', () => {
    ...
    it('should repair broken tool argument JSON', () => {
        ...
        const parsed = safeParseJson(malformed);
    ...
    it('should decode dual-track SSE stream with reasoning and tool_calls', () => {
        ...
        const events = [];
        // Stream reasoning
        decoder.processChunk({ choices: [{ delta: { reasoning_content: 'Let me think...' } }] }, { onEvent: (e) => events.push(e) });
        // Stream content
        decoder.processChunk({ choices: [{ delta: { content: 'Starting execution.' } }] }, { onEvent: (e) => events.push(e) });
        // Stream tool call
