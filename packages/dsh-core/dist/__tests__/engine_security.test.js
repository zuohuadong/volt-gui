...
function completionStream(chunks) {
    return {
...
function stubCompletions(engine, responses) {
    engine.client = {
        chat: {
            completions: {
                create: async () => {
...
async function collectTurn(engine, prompt, maxSteps) {
    const events = [];
    for await (const event of engine.runTurn(prompt, { maxSteps }))
        events.push(event);
    return events;
}
...
//# sourceMappingURL=engine_security.test.js.map