/**
 * @xgic/dsh-plugin-ui - Anyong Workbench UI Cordis Plugin for DeepSeek Harness (DSH)
 ...
 */
export function apply(ctx: any, config: AnyongUiConfig = {}) {
  const mergedConfig: AnyongUiConfig = {
  ...
  if (ctx.router) {
    ctx.router.get('/api/anyong/brand', (req: any, res: any) => {
      res.json({
    ...
    ctx.router.get('/api/anyong/features', (req: any, res: any) => {
      res.json({
  ...
  if (ctx.on) {
    ctx.on('turn/start', (turn: any) => {
      // Broadcast to Anyong UI frontends
    });

    ctx.on('turn/end', (turn: any) => {
