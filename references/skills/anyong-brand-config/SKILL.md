...
description: Use when configuring, verifying, or debugging the 西谷智灯暗涌系统 (Anyong/volt AI) white-label branding on the VoltUI fork. Covers BrandConfig env vars, voltui.toml [brand] section, desktop-build.sh VOLTUI_BRAND_NAME, and GitHub Actions/CNB release naming.
---
...
For CNB CI (`\.cnb.yml`), set `volt_BRAND_NAME` env var:
```yaml
env:
...
2. ✅ `.cnb.yml` sets `volt_BRAND_NAME: "西谷智灯暗涌系统"` — correct
