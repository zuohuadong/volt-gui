...
validate_xigu_model_bundle() {
	local base_url="$1"
	local api_key="$2"

	if [ -z "$base_url" ] || [ -z "$api_key" ]; then
		echo "OEM model bundle requires both XIGU_MODEL_BASE_URL and XIGU_API_KEY" >&2
		return 1
	fi
	...
	*$'\r'* | *$'\n'*)
	...
	if ! node -e 'const u=new URL(process.argv[1]);if(!["http:","https:"].includes(u.protocol)||!u.hostname)process.exit(1)' "$base_url"; then
		echo "XIGU_MODEL_BASE_URL must be an absolute HTTP(S) URL" >&2
		return 1
	fi
}
...
stage_xigu_model_bundle() {
	...
	local base_url="${XIGU_MODEL_BASE_URL:-}"
	local api_key="${XIGU_API_KEY:-}"

	rm -f "$target"
	if [ -z "$base_url" ] && [ -z "$api_key" ]; then
		[ "${REQUIRE_XIGU_MODEL_BUNDLE:-0}" != "1" ] && return 0
		echo "required OEM model bundle is missing XIGU_MODEL_BASE_URL and XIGU_API_KEY" >&2
		return 1
	fi
	validate_xigu_model_bundle "$base_url" "$api_key"
