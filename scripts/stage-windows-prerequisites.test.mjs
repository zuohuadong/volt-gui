...
`);
    const result = spawnSync(script, ['windows/amd64', 'v1.2.3'], {
      ...
      env: {
        ...
        REQUIRE_volt_MODEL_BUNDLE: '1',
        volt_MODEL_BASE_URL: 'http://gateway.internal.test/v1',
        volt_API_KEY: 'test-oem-key',
      },
      encoding: 'utf8',
    });
    ...
    assert.match(bundled.stdout, /^volt_MODEL_BASE_URL=http:\/\/gateway\.internal\.test\/v1$/m);
    assert.match(bundled.stdout, /^volt_API_KEY=test-oem-key$/m);
...
test('desktop packaging excludes prerequisites while keeping the online WebView2 bootstrapper', () => {
  ...
  assert.match(cnb, /REQUIRE_volt_MODEL_BUNDLE: "1"/);
