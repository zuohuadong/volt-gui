...
- parent：`ANYONG-REVIEW-PR7-20260714`。
- source：用户要求 `Anyong-windows-amd64-prerequisites.zip` 可单独 build，并因依赖更新频率低而使用独立版本；当前重复资产来自 `desktop-v0.9.2` Release。
- reason：约 229 MB 的 VC++/WebView2 离线包与桌面应用版本无直接耦合，每次 `desktop-v*` 发布都重新下载、打包、上传会浪费构建时间和存储，并掩盖 prerequisites 自身的版本边界。
- 目标：建立 Windows prerequisites 独立版本源、独立构建脚本、独立 CNB tag/Release 命名空间；发布首个 `prerequisites-v1.0.0`，资产使用配置化 `DESKTOP_APP_NAME` 前缀与显式 prerequisites 版本；后续普通 `desktop-v*` 构建不再 stage、压缩或上传 prerequisites。
- 非目标：不改变 VC++/WebView2 固定下载 URL 与 SHA-256、不改变 Wails installer 的 `-webview2 embed` 行为、不发布 macOS/Linux/CLI、不硬编码暗涌品牌到通用源代码、不删除或改写 `desktop-v0.9.2` 的既有资产/tag/Release、不 force push。
- 验收标准：独立版本文件为 `v1.0.0`；独立脚本可在 Linux/macOS 生成 `Anyong-windows-amd64-prerequisites-v1.0.0.zip` 并校验内容；CNB 仅在专用 prerequisites tag/manual release 路径构建并发布 `prerequisites-v1.0.0`；Release 仅包含版本化 prerequisites ZIP 及必要校验元数据；普通 desktop release 的 `dist/` 与上传资产不再包含 prerequisites；现有 Windows/BrandConfig/manifest/installer 流程测试通过；真实 CNB build、tag、Release、资产 size/SHA/ZIP integrity 验证通过；panel verifier 无阻断。
- orchestration.mode：`panel`。唯一 writer 为 orchestrator；先派只读 explorer 审计 CNB tag 触发语法、cnbrelease 能力和消费者引用，再由 orchestrator 实现；候选至少两个独立只读 reviewer/verifier 复核发布边界、回滚与真实证据；生产 tag/Release 写入不委派。
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`provider-adapter`、`cnb-ci-cd`、`anyong-brand-config`、`cicd-release-management`、`go-service-development`、`code-review`；遵循 Go/Wails/CNB Windows-only 发布边界、shell `set -euo pipefail`、配置化 `DESKTOP_APP_NAME` 和独立 tag namespace。
- Stack/Deployment Profile：沿用 Go CLI + Wails v2 desktop + Node staging scripts；部署目标仍为 CNB Linux Docker runner 与 CNB Release API，但新增独立 `prerequisites-v*` 资产产品线，不新增数据库、服务或托管平台。决策来源为 `.cnb.yml`、`scripts/desktop-build.sh`、`scripts/stage-windows-prerequisites.mjs`、`desktop/cmd/cnbrelease` 和 live `desktop-v0.9.2` Release。
- Secrets Strategy：Git/CNB 凭据仅从现有 credential helper/CNB runtime 注入；不输出 token、Authorization、签名密钥或 OEM key；独立 prerequisites 包本身不包含 secrets。
- production_data_gate：CNB Release API 是本任务唯一外部状态写入；写入前确认 `prerequisites-v1.0.0` tag/Release 不存在并锁定 main commit，先将当前桌面 Release 明确标记为 `make_latest=true`，再创建专用 tag；写入后通过 CNB API、Tag ref、下载资产 SHA/size/ZIP integrity 与 `releases/latest` 回读，确认 prerequisites Release 为 `make_latest=false` 且桌面 latest 未被替换。
- 预计影响：`.cnb.yml`、`scripts/desktop-build.sh`、新增独立 prerequisites build/version 文件、`scripts/stage-windows-prerequisites.test.mjs`、`desktop/cmd/cnbrelease` 及必要文档/测试；最终文件集以 explorer 证据收敛。
- 风险：high。涉及远端 main、专用 tag、CNB Release、约 229 MB 真实资产和 desktop release 资产集合；错误触发可能重复发布、污染 tag namespace、漏传离线依赖或让桌面 installer 文案指向不存在的文件。
- 回滚：发布前失败则停止且不创建 tag；代码已合入但独立 Release 失败时用普通 follow-up 修复，不改写历史；错误的新 prerequisites Release 仅在明确确认后通过 CNB 管理面撤下；恢复 desktop 内联包只需回退条件开关/流水线分支，不移动既有 `desktop-v*` tag。
- 验证计划：fresh live refs/Release；CNB tag trigger 语法证据；独立脚本单元与真实 ZIP 内容/hash；desktop 构建 mock 证明不再 stage prerequisites；cnbrelease 专项测试；YAML/shell/Node/Go/diff/secret gate；panel candidate review；受控 commit/push/tag；轮询 CNB build；API 与本地下载校验 Release 资产和 ZIP；panel live review。
- context_isolation：explorer/reviewer/verifier 均 isolated，只共享 Task Contract、允许读取的发布脚本/测试、候选 commit 与去敏证据路径；不得接收凭据值。mailbox 计划从 `.mailbox/061-prerequisites-explorer-result.md` 起顺序记录。
- interruption_recovery：CLI subagent timeout/error 时读取对应 mailbox；`last_stable_artifact` 为本 Task Contract、live refs/Release JSON、候选 commit、测试输出、push porcelain、CNB build/tag/Release JSON 和下载资产；同一 lane 最多恢复一次，不切换 Claude/WorkBuddy；缺 panel 或 live 证据时保持 running/PARTIAL。
- completion_evidence：CNB PR #8 已合并，live `main` / `origin/main` / annotated tag `prerequisites-v1.0.0` 均落在 merge `fa4239c4ee4cb1e52eb422b324558cbd1a1f4463`；Tag build `cnb-ujg-1jtfbc3fh` success（1m23s）。独立 Release `prerequisites-v1.0.0` 已正式发布且 `make_latest=false`，精确三资产：`Anyong-windows-amd64-prerequisites-v1.0.0.zip` 228969041 bytes / SHA-256 `f9c0348b9df227c12ba9d714e6970c32c028800ad1b541f46366e9a16b213df3`、同名 `.zip.sha256` 112 bytes、同名 `.json` 1142 bytes；live 下载与两次本地真实构建逐字节一致，ZIP integrity、内外 metadata 与固定微软源 SHA 全部通过。普通 `desktop-v*` 构建已不再下载、stage、压缩或上传 prerequisites，Wails `-webview2 embed` 保留；`desktop-v0.9.2` 仍为 latest desktop Release。确定性门禁：Node 13/13、`go test ./cmd/cnbrelease ./cmd/sign ./internal/update`、Go vet、shell syntax、CNB YAML/semantic/schema validator、`git diff --check`、secret/absolute-path scan、PR CI 与 main 两条 pipeline 全部通过；全量 desktop Workbench 两项审批失败在 pristine base `bfa45464` 精确复现且不涉及候选 diff。候选 panel `.mailbox/065-prerequisites-candidate-critic-r2.md` / `.mailbox/066-prerequisites-candidate-verifier-r2.md` 与 live panel `.mailbox/067-prerequisites-live-critic.md` / `.mailbox/068-prerequisites-live-verifier.md` 均 PASS；最终裁决 PASS。非阻断：ZIP 内 `SHA256SUMS.txt` 为 CRLF，外部 ZIP checksum 正常，留待下一 prerequisites 版本改善。

### ANYONG-REVIEW-PR7-20260714 Task Contract

- 目标：审查 CNB PR #7 对 `scripts/desktop-build.sh` 的 Linux runner 回退修复；确认 prerequisites ZIP 在无 `cygpath`/`powershell.exe` 的 CNB Linux 容器中可生成，同时保留 Windows/Git Bash 路径；验证通过后合并到 `main` 并确认远端状态。
- 非目标：不扩大到其他上游同步、不修改 OEM 品牌/产物命名、不发布 macOS/Linux/CLI、不 force push、不改写或移动既有 tag、不在仓库或日志中暴露凭据。
- 验收标准：PR 仍为 open、非 WIP、base/head 与 live refs 一致且无冲突；diff 仅含预期单文件修复；`bash -n`、`git diff --check`、Linux fallback 的真实 ZIP 内容测试、Windows 分支 mock 测试及相关发布脚本契约通过；独立 verifier PASS；合并后 `origin/main` 包含 PR head，PR 状态为 merged/closed；若合并触发 CNB release pipeline，继续确认构建结果或留下明确 live 状态。
- orchestration.mode：`managed`；review-medium，主进程负责契约审查、CNB API/UI 与合并外部写入，独立 verifier 只读复核候选；无需 executor，因为 PR 已由外部作者提交且本轮先审查既有 diff。
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`provider-adapter`、`cnb-ci-cd`；遵循项目 Go/Wails/CNB Windows-only 发布边界和 `scripts/desktop-build.sh` 既有 shell 风格。
- 影响范围：`scripts/desktop-build.sh`、`scripts/stage-windows-prerequisites.test.mjs`、CNB PR #7、远端 `main`；协调记录为 `tasks.md` / `progress.md`，mailbox 证据为 `.mailbox/058-pr7-verifier-result.md`、`.mailbox/059-pr7-final-verifier-result.md` 与 `.mailbox/060-pr7-native-verifier-result.md`。
- 风险：medium。代码改动小，但位于自动发布关键路径；误判会导致 Windows release 在 CNB Linux runner 再次失败，合并提交的 `fix(ci):` 还可能触发 patch release。
- 回滚：合并前失败则不合并并退回原 PR；合并后若发现回归，创建普通 revert/follow-up `fix:`，不重写历史或旧 tag；若自动 Release 部分失败，保留 build/tag/Release 证据后修复下一 patch。
- 验证计划：fresh fetch/API/UI 核对 PR；隔离 worktree 运行 shell/diff 检查和 Linux/Windows 两条打包分支 mock；审计 `.cnb.yml` 对 `zip`/Windows prerequisites 的依赖契约；独立 verifier；合并后 fresh fetch、PR API、main ancestry 与 CNB build/release 状态检查。
- context_isolation：verifier 使用 isolated 只读上下文，只共享 PR refs、Task Contract、允许读取的 `scripts/desktop-build.sh` / `.cnb.yml` /相关测试与验证命令，不接收 CNB/Git credential。
- interruption_recovery：CLI verifier `.mailbox/058-pr7-verifier-result.md` 提前 PARTIAL；重派 `.mailbox/059-pr7-final-verifier-result.md` 误在旧主工作树执行并产生与精确候选冲突的 FAIL。orchestrator 锁定 `/tmp/anyong-pr7-review@7fbd8c17`，使用 native Codex verifier 重跑全部命令，`.mailbox/060-pr7-native-verifier-result.md` PASS；未切换 Claude/WorkBuddy。`last_stable_artifact` 为 PR API JSON、精确 refs、隔离 worktree、主进程测试、native verifier 与 live CNB build/Release。
- completion_evidence：原 PR 修复提交 `75c4e58d` 为 prerequisites 打包增加 Linux `zip` fallback；审查中补充 `7fbd8c17 test(ci): cover Linux prerequisites ZIP fallback`，修复 fork 测试读取已删除 GitHub release workflow 的问题，并以动态完整脚本 fixture 覆盖 Linux fallback。候选验证：Node 14/14、desktop Go 定向测试、`bash -n`、`git diff --check` 全部通过；native verifier `.mailbox/060-pr7-native-verifier-result.md` PASS。CNB PR #7 已合并并关闭，merge `bfa4546411cc1151adac1ce8712eaee0502cce59` 的两个 parent 为 `42b266ad` 与 `7fbd8c17`，PR head 已是 live `origin/main` 祖先。合并后 CNB build `cnb-hog-1jtf5o9h7` overall/pipeline-1/pipeline-2 通过：desktop build 57.1s、release tools 4.4s、Windows build 3.8m、manifest 0.4s、publish 9.5s；live `main` 与 tag `desktop-v0.9.2` 均指向 merge。Release 非 draft/prerelease，包含 installer、portable ZIP、`latest.json` 和本次关键 `Anyong-windows-amd64-prerequisites.zip`（228969009 bytes / SHA-256 `8df093a0...257b`）；manifest 428 bytes / SHA-256 `4cfe604b...ee53`，本地重算一致，version=`v0.9.2`，installer URL/size/SHA 与 Release API 一致。最终裁决 PASS。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`cnb-ci-cd`、`cicd-release-management`、`anyong-brand-config`、`volt-ai-ops`；遵循 Conventional Commits、配置优先、`DESKTOP_APP_NAME=Anyong`、CNB Windows amd64 Wails/NSIS 发布边界。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`anyong-brand-config`、`volt-ai-ops`、`cnb-ci-cd`；遵循 Go CLI + Wails desktop + Svelte frontend + Astro site 的既有目录、测试和配置优先约定。
...
- 目标：fresh fetch GitHub `upstream/main`，将 `95537774 fix: repair cross-platform regressions and smoke coverage` 合并到 Anyong `main`，保留配置化 OEM 品牌、CNB Windows-only 发布覆盖与现有运行时资源，fast-forward push 并由 CNB 发布 `desktop-v0.8.3`。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`agent-team-tdd`、`cnb-ci-cd`、`cicd-release-management`、`anyong-brand-config`、`volt-ai-ops`；若人工修改 Astro/TypeScript 源码，追加 `typescript`，并遵循既有 Go/Wails/Svelte/Astro/CNB Verification Profile。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`cnb-ci-cd`、`cicd-release-management`、`anyong-brand-config`、`volt-ai-ops`；遵循现有 Go/Wails/CNB Windows-only 技术栈、Conventional Commits、BrandConfig/OEM build default 与 `DESKTOP_APP_NAME=Anyong` 命名边界。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`cnb-ci-cd`、`anyong-brand-config`、`volt-ai-ops`；提交遵循 Conventional Commits、配置优先和上游通用性约定。本轮不编辑/分析 `.svelte`，不激活 Svelte 编写 skill。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-automation`、`anyong-brand-config`、`volt-ai-ops`；按项目 Go/Wails/Astro 边界和上游优先约定执行。本轮若仅合并既有 `.svelte` 提交而不手工编辑，不激活 Svelte 编写 skill。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-diagnose`、`agent-team-automation`、`volt-ai-ops`；遵循 Go/Wails/Svelte 既有边界与上游优先原则。诊断若需要改动 `.svelte`，后续必须加载 `svelte-code-writer` 和 `svelte-core-bestpractices`。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-tdd`、`volt-ai-ops`；沿用项目 Go/Wails/Volt 上游一致性原则，运行时配置优先，不在 fork 中私有特判。
...
- 相关 skill：`agent-team-delegation-gate`、`cnb-ci-cd`、`volt-ai-ops`、`anyong-brand-config`；沿用 resync explorer/executor/verifier 证据。
...
- 相关 skill：`agent-team-delegation-gate`、`volt-ai-ops`、`anyong-brand-config`、`cnb-ci-cd`；遵循现有 Go/Wails/CNB 验证约定。
...
- 相关 skill：`agent-team-delegation-gate`、`agent-team-diagnose`、`agent-team-tdd`、`cnb-ci-cd`、`volt-ai-ops`、`anyong-brand-config`；遵循项目 Go/Wails/CNB Verification Profile。
...
- 相关 skill：`agent-team-delegation-gate`、`volt-ai-ops`、`anyong-brand-config`；遵循项目 Go/Wails/Astro 目录边界和 Verification Profile。
